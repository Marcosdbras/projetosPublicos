...
type
  TfrmPesqsitb = class(TForm)
    ...
    procedure dbgSITBKeyPress(Sender: TObject; var Key: Char);
    procedure dbgSITBEnter(Sender: TObject);
    ...
    procedure dbgSITBExit(Sender: TObject);
    ...
    procedure dbgSITBKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses uGeral, uPrincipal, uDados, uMRsitb;
...
procedure TfrmPesqsitb.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqsitb.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqsitb.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqsitb.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqsitb.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmPesqsitb.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqsitb.dbgSITBKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
   if frmdados.Cds_SITB.State in [dsinsert, dsedit] then
      frmdados.Cds_SITB.Post;
   //endi
//endi
end;

procedure TfrmPesqsitb.dbgSITBEnter(Sender: TObject);
begin
tabela := 'SITB';
end;

procedure TfrmPesqsitb.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqsitb.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqsitb.spdfiltrarClick(Sender: TObject);
var
...
begin
  ...
  try
    with frmdados.Dbx_SITB do
       begin
         active := false;
         sql.Clear;
         if sCompo <> '' then
            sql.Add('select * from SITB where '+sCompo+' order by '+sClassificar)
         else
            sql.Add('select * from SITB order by '+sClassificar);
         //endi
         active := true;
            ...
            begin
              if sDesc <> '' then
                 begin
                   active := false;
                   sql.Clear;
                   sql.Add('select * from SITB where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by '+sClassificar);
                   active := true;
                 end;
              //endi
            end;
...
procedure TfrmPesqsitb.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqsitb.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   frmdados.cds_SITB.Locate(frmpesqsitb.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure TfrmPesqsitb.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqsitb.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqsitb.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqsitb.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
if key = #13 then
   begin
     ...
     if edicodigoi.Text <> '' then
        if not frmdados.cds_SITB.Locate('codigo',edicodigoi.text,[])then
...
procedure TfrmPesqsitb.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
procedure TfrmPesqsitb.dbgSITBExit(Sender: TObject);
begin
  if frmdados.Cds_SITB.State in [dsinsert, dsedit] then
     frmdados.Cds_SITB.Post;
  //endi
...
procedure TfrmPesqsitb.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqsitb.spdIncluir171Click(Sender: TObject);
begin
tabela := 'SITB';
dbgSITB.SetFocus;
frmdados.Cds_SITB.Append;
end;

procedure TfrmPesqsitb.spdantClick(Sender: TObject);
begin
  frmdados.Cds_SITB.Prior;

end;

procedure TfrmPesqsitb.spdproxClick(Sender: TObject);
begin
frmdados.Cds_SITB.Next;
end;

procedure TfrmPesqsitb.spdExcluir173Click(Sender: TObject);
begin
tabela := 'SITB';
...
procedure TfrmPesqsitb.dbgSITBKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmPesqsitb.spdrelatorios176Click(Sender: TObject);
begin
    frmMRsitb := tfrmMRsitb.Create(self);
    frmMRsitb.ShowModal;
    frmMRsitb.Free;
end;
