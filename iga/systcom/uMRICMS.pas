...
var
  frmMRICMS: TfrmMRICMS;

implementation

uses uGeral, uPrincipal, uDados, urelICMS, urelICMSs ;

{$R *.DFM}

procedure TfrmMRICMS.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 1 then
   begin
   frmrelICMSs := tfrmrelICMSs.Create(self);
   frmrelICMSs.relatorio.Preview;
   frmrelICMSs.Free;
   end;
//endi
...
procedure TfrmMRICMS.FormShow(Sender: TObject);
begin
...
procedure TfrmMRICMS.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRICMS.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRICMS.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRICMS.FormClose(Sender: TObject; var Action: TCloseAction);
begin
