...
var
  frmMRmobrad: TfrmMRmobrad;

implementation

uses uGeral, uPrincipal, uDados, urelmobrad ,urelmobrads;

{$R *.DFM}

procedure TfrmMRmobrad.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelmobrad := tfrmrelmobrad.Create(self);
     frmrelmobrad.relatorio.Preview;
     frmrelmobrad.Free;
   end;
//endi
...
if rgbtiporel.ItemIndex = 2 then
   begin
    frmrelmobrads := tfrmrelmobrads.Create(self);
     frmrelmobrads.relatorio.Preview;
     frmrelmobrads.Free;
   end;
//endi
...
procedure TfrmMRmobrad.FormShow(Sender: TObject);
begin
...
procedure TfrmMRmobrad.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRmobrad.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRmobrad.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRmobrad.FormClose(Sender: TObject; var Action: TCloseAction);
begin
