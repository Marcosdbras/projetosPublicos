...
type
  TfrmPesqSITA = class(TForm)
    ...
    procedure dbgSITAKeyPress(Sender: TObject; var Key: Char);
    procedure dbgSITAEnter(Sender: TObject);
    ...
    procedure dbgSITAExit(Sender: TObject);
    ...
    procedure dbgSITAKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses uGeral, uPrincipal, uDados ,uMRsita;
...
procedure TfrmPesqSITA.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqSITA.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqSITA.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqSITA.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqSITA.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmPesqSITA.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqSITA.dbgSITAKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
   if frmdados.Cds_SITA.State in [dsinsert, dsedit] then
      frmdados.Cds_SITA.Post;
   //endi
//endi
end;

procedure TfrmPesqSITA.dbgSITAEnter(Sender: TObject);
begin
tabela := 'SITA';
end;

procedure TfrmPesqSITA.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqSITA.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqSITA.spdfiltrarClick(Sender: TObject);
var
...
begin
  ...
  try
    with frmdados.Dbx_SITA do
       begin
         active := false;
         sql.Clear;
         if sCompo <> '' then
            sql.Add('select * from SITA where '+sCompo+' order by '+sClassificar)
         else
            sql.Add('select * from SITA order by '+sClassificar);
         //endi
         active := true;
            ...
            begin
              if sDesc <> '' then
                 begin
                   active := false;
                   sql.Clear;
                   sql.Add('select * from SITA where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by '+sClassificar);
                   active := true;
                 end;
              //endi
            end;
...
procedure TfrmPesqSITA.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqSITA.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   frmdados.cds_SITA.Locate(frmPesqSITA.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure TfrmPesqSITA.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqSITA.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqSITA.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqSITA.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
if key = #13 then
   begin
     ...
     if edicodigoi.Text <> '' then
        if not frmdados.cds_SITA.Locate('codigo',edicodigoi.text,[])then
...
procedure TfrmPesqSITA.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
procedure TfrmPesqSITA.dbgSITAExit(Sender: TObject);
begin
  if frmdados.Cds_SITA.State in [dsinsert, dsedit] then
     frmdados.Cds_SITA.Post;
  //endi
...
procedure TfrmPesqSITA.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqSITA.spdIncluir171Click(Sender: TObject);
begin
tabela := 'SITA';
dbgSITA.SetFocus;
frmdados.Cds_SITA.Append;
end;

procedure TfrmPesqSITA.spdantClick(Sender: TObject);
begin
  frmdados.Cds_SITA.Prior;

end;

procedure TfrmPesqSITA.spdproxClick(Sender: TObject);
begin
frmdados.Cds_SITA.Next;
end;

procedure TfrmPesqSITA.spdExcluir173Click(Sender: TObject);
begin
tabela := 'SITA';
...
procedure TfrmPesqSITA.dbgSITAKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmPesqSITA.spdrelatorios176Click(Sender: TObject);
begin
    frmMRsita :=tfrmMRsita.create(self);
    frmMRsita.Showmodal;
    frmMRsita.Free;
end;
