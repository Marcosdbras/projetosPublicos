...
type
  TfrmPesqvii = class(TForm)
    ...
    procedure dbgViiKeyPress(Sender: TObject; var Key: Char);
    procedure dbgViiEnter(Sender: TObject);
    ...
    procedure dbgViiExit(Sender: TObject);
    ...
    procedure dbgViiKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
var
  frmPesqvii: TfrmPesqvii;

implementation

uses uGeral, uPrincipal, uDados , uMRvii;

{$R *.DFM}

procedure TfrmPesqvii.FormShow(Sender: TObject);
var
   vardir: String;
begin
...
procedure TfrmPesqvii.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqvii.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqvii.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqvii.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqvii.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmPesqvii.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqvii.dbgViiKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
   if frmdados.Cds_Vii.State in [dsinsert, dsedit] then
      frmdados.Cds_Vii.Post;
   //endi
//endi
end;

procedure TfrmPesqvii.dbgViiEnter(Sender: TObject);
begin
tabela := 'vii';
end;

procedure TfrmPesqvii.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqvii.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqvii.spdfiltrarClick(Sender: TObject);
var
...
begin
  ...
  try
    with frmdados.Dbx_Vii do
       begin
         active := false;
         sql.Clear;
         if sCompo <> '' then
            sql.Add('select * from Vii where '+sCompo+' order by '+sClassificar)
         else
            sql.Add('select * from Vii order by '+sClassificar);
         //endi
         active := true;
            ...
            begin
              if sDesc <> '' then
                 begin
                   active := false;
                   sql.Clear;
                   sql.Add('select * from Vii where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by '+sClassificar);
                   active := true;
                 end;
              //endi
            end;
...
procedure TfrmPesqvii.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqvii.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   frmdados.cds_Vii.Locate(frmpesqVii.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure TfrmPesqvii.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqvii.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqvii.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqvii.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
if key = #13 then
   begin
     ...
     if edicodigoi.Text <> '' then
        if not frmdados.cds_Vii.Locate('codigo',edicodigoi.text,[])then
...
procedure TfrmPesqvii.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
procedure TfrmPesqvii.dbgViiExit(Sender: TObject);
begin
  if frmdados.Cds_Vii.State in [dsinsert, dsedit] then
     frmdados.Cds_Vii.Post;
  //endi
...
procedure TfrmPesqvii.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqvii.spdIncluir171Click(Sender: TObject);
begin
tabela := 'vii';
dbgVii.SetFocus;
frmdados.Cds_Vii.Append;
end;

procedure TfrmPesqvii.spdantClick(Sender: TObject);
begin
  frmdados.Cds_Vii.Prior;

end;

procedure TfrmPesqvii.spdproxClick(Sender: TObject);
begin
frmdados.Cds_Vii.Next;
end;

procedure TfrmPesqvii.spdExcluir173Click(Sender: TObject);
begin
tabela := 'vii';
if application.MessageBox('Deseja Realmente Excluir Esse Registro?','Atenção',mb_YesNo + mb_DefButton2) = idYes then
   begin

     frmdados.Cds_Vii.Delete;
...
procedure TfrmPesqvii.dbgViiKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmPesqvii.spdrelatorios176Click(Sender: TObject);
begin
    frmMRvii:= tfrmMRvii.create(self);
    frmMRvii.Showmodal;
    frmMRvii.Free;
end;
