...
uses uGeral, uPrincipal, uDados, uMRmunic;
...
procedure TfrmPesqMunic.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqMunic.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqMunic.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqMunic.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqMunic.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmPesqMunic.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqMunic.dbgEstKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
   if frmdados.Cds_Munic.State in [dsinsert, dsedit] then
      frmdados.Cds_Munic.Post;
   //endi
//endi
end;

procedure TfrmPesqMunic.dbgEstEnter(Sender: TObject);
begin
tabela := 'Munic';
end;

procedure TfrmPesqMunic.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqMunic.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqMunic.spdfiltrarClick(Sender: TObject);
var
...
begin
  ...
  try
    with frmdados.Dbx_Munic do
       begin
         active := false;
         sql.Clear;
         if sCompo <> '' then
            sql.Add('select * from Munic where '+sCompo+' order by '+sClassificar)
         else
            sql.Add('select * from Munic order by '+sClassificar);
         //endi
         active := true;
            ...
            begin
              if sDesc <> '' then
                 begin
                   active := false;
                   sql.Clear;
                   sql.Add('select * from Munic where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by '+sClassificar);
                   active := true;
                 end;
              //endi
            end;
...
procedure TfrmPesqMunic.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqMunic.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   frmdados.cds_Munic.Locate(frmpesqMunic.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure TfrmPesqMunic.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqMunic.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqMunic.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqMunic.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
if key = #13 then
   begin
     ...
     if edicodigoi.Text <> '' then
        if not frmdados.cds_Munic.Locate('codigo',edicodigoi.text,[])then
...
procedure TfrmPesqMunic.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
procedure TfrmPesqMunic.dbgEstExit(Sender: TObject);
begin
  if frmdados.Cds_Munic.State in [dsinsert, dsedit] then
     frmdados.Cds_Munic.Post;
  //endi
...
procedure TfrmPesqMunic.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqMunic.spdIncluir171Click(Sender: TObject);
begin
tabela := 'Munic';
dbgEst.SetFocus;
...
procedure TfrmPesqMunic.spdantClick(Sender: TObject);
begin
  frmdados.Cds_Munic.Prior;

end;

procedure TfrmPesqMunic.spdproxClick(Sender: TObject);
begin
frmdados.Cds_Munic.Next;
end;

procedure TfrmPesqMunic.spdExcluir173Click(Sender: TObject);
begin
tabela := 'Munic';
...
procedure TfrmPesqMunic.dbgEstKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmPesqMunic.spdetqClick(Sender: TObject);
begin
    frmMRmunic := tfrmMRmunic.create(self);
    frmMRmunic.Showmodal;
    frmMRmunic.free;
end;
