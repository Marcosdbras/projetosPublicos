...
type
  TfrmPesqtcc = class(TForm)
    ...
    procedure dbgtccKeyPress(Sender: TObject; var Key: Char);
    procedure dbgtccEnter(Sender: TObject);
    ...
    procedure dbgtccExit(Sender: TObject);
    ...
    procedure dbgtccKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
var
  frmPesqtcc: TfrmPesqtcc;

implementation

uses uGeral, uPrincipal, uDados, uMRtcc;

{$R *.DFM}

procedure TfrmPesqtcc.FormShow(Sender: TObject);
var
   vardir: String;
begin
...
procedure TfrmPesqtcc.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqtcc.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqtcc.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqtcc.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqtcc.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmPesqtcc.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqtcc.dbgtccKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
   if frmdados.Cds_SituCadCli.State in [dsinsert, dsedit] then
      frmdados.Cds_SituCadCli.Post;
   //endi
//endi
end;

procedure TfrmPesqtcc.dbgtccEnter(Sender: TObject);
begin
tabela := 'SituCadCli';
end;

procedure TfrmPesqtcc.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqtcc.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqtcc.spdfiltrarClick(Sender: TObject);
var
...
begin
  ...
  try
    with frmdados.Dbx_SituCadCli do
       begin
         active := false;
         sql.Clear;
         if sCompo <> '' then
            sql.Add('select * from SituCadCli where '+sCompo+' order by '+sClassificar)
         else
            sql.Add('select * from SituCadCli order by '+sClassificar);
         //endi
         active := true;
            ...
            begin
              if sDesc <> '' then
                 begin
                   active := false;
                   sql.Clear;
                   sql.Add('select * from SituCadCli where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by '+sClassificar);
                   active := true;
                 end;
              //endi
            end;
...
procedure TfrmPesqtcc.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqtcc.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   frmdados.cds_SituCadCli.Locate(frmpesqtcc.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure TfrmPesqtcc.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqtcc.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqtcc.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqtcc.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
if key = #13 then
   begin
     ...
     if edicodigoi.Text <> '' then
        if not frmdados.cds_SituCadCli.Locate('codigo',edicodigoi.text,[])then
...
procedure TfrmPesqtcc.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
procedure TfrmPesqtcc.dbgtccExit(Sender: TObject);
begin
  if frmdados.Cds_SituCadCli.State in [dsinsert, dsedit] then
     frmdados.Cds_SituCadCli.Post;
  //endi
...
procedure TfrmPesqtcc.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqtcc.spdIncluir171Click(Sender: TObject);
begin
tabela := 'SituCadCli';
dbgtcc.SetFocus;
frmdados.Cds_SituCadCli.Append;
end;

procedure TfrmPesqtcc.spdantClick(Sender: TObject);
begin
  frmdados.Cds_SituCadCli.Prior;

end;

procedure TfrmPesqtcc.spdproxClick(Sender: TObject);
begin
frmdados.Cds_SituCadCli.Next;
end;

procedure TfrmPesqtcc.spdExcluir173Click(Sender: TObject);
begin
tabela := 'SituCadCli';
if application.MessageBox('Deseja Realmente Excluir Esse Registro?','Atenção',mb_YesNo + mb_DefButton2) = idYes then
   begin

     frmdados.Cds_SituCadCli.Delete;
...
procedure TfrmPesqtcc.dbgtccKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmPesqtcc.spdrelatorios176Click(Sender: TObject);
begin
    frmMRtcc := tfrmMRtcc.create(self);
    frmMRtcc.Showmodal;
    frmMRtcc.Free;
end;
