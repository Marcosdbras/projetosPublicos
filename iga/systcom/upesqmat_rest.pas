...
type
  TfrmPesqMat_Rest = class(TForm)
    ...
    procedure mostrararq_ext2;
    procedure mostrararq_ext3;
    procedure mostrararq_ext4;
...
imgfoto3.Visible := false;
imgfoto4.Visible := false;
ediid.SetFocus;
...
if sOpcao <> 'E' then
   begin
     if (frmdados.Cds_Mat_Restaurar.State in [dsinsert, dsedit]) and (edidescricao.Text <> '') then
...
procedure tfrmPesqMat_Rest.mostrararq_ext2;
var
 J : TJPEGImage;
 figura : tbitmap;
 S : TMemoryStream;
...
begin
 ...
 with frmdados do
   begin
      ...
      if cds_Mat_Restaurarfoto2.BlobSize = 0 then exit;

      //se nã há informação sobre o tipo do arquivo, sai...
      if cds_Mat_Restaurarfilename2.AsString = '' then exit;


      //Recupera a extensão do arquivo
      Extensao := uppercase(extractfileext(cds_Mat_Restaurarfilename3.AsString));

      //se for image JPG ou BMP carrega para img
      if pos(extensao,'.JPG .JPEG .BMP') <> 0 then
         begin

           Stream := cds_Mat_Restaurar.CreateBlobStream(cds_Mat_Restaurarfoto2, bmread);
           try
             if extensao = '.BMP' then
                begin
                  figura := tbitmap.Create;
         ...
         begin
           ...
           if not directoryexists(temppath) then
              ...
              //endi
           //endi
           temppath := temppath + '\'+cds_Mat_Restaurarfoto2.AsString;

           //Grava bloco no arquivo temporário
           cds_Mat_Restaurarfoto2.SaveToFile(temppath);
...
procedure tfrmPesqMat_Rest.mostrararq_ext3;
var
 J : TJPEGImage;
 figura : tbitmap;
 S : TMemoryStream;
...
begin
 ...
 with frmdados do
   begin
      ...
      if cds_Mat_Restaurarfoto3.BlobSize = 0 then exit;

      //se nã há informação sobre o tipo do arquivo, sai...
      if cds_Mat_Restaurarfilename3.AsString = '' then exit;
      ...
      if pos(extensao,'.JPG .JPEG .BMP') <> 0 then
         begin
           ...
           Stream := cds_Mat_Restaurar.CreateBlobStream(cds_Mat_Restaurarfoto3, bmread);
           try
             if extensao = '.BMP' then
                begin
                  figura := tbitmap.Create;
                  try
                    figura.LoadFromStream(stream);
                    imgfoto3.Picture.Assign(figura)
                  finally
         ...
         begin
           ...
           if not directoryexists(temppath) then
              ...
              //endi
           //endi
           temppath := temppath + '\'+cds_Mat_Restaurarfoto3.AsString;

           //Grava bloco no arquivo temporário
           cds_Mat_Restaurarfoto3.SaveToFile(temppath);
...
procedure tfrmPesqMat_Rest.mostrararq_ext4;
var
 J : TJPEGImage;
 figura : tbitmap;
 S : TMemoryStream;
...
begin
 ...
 with frmdados do
   begin
      ...
      if cds_Mat_Restaurarfoto4.BlobSize = 0 then exit;

      //se nã há informação sobre o tipo do arquivo, sai...
      if cds_Mat_Restaurarfilename4.AsString = '' then exit;


      //Recupera a extensão do arquivo
      Extensao := uppercase(extractfileext(cds_Mat_Restaurarfilename4.AsString));

      //se for image JPG ou BMP carrega para img
      if pos(extensao,'.JPG .JPEG .BMP') <> 0 then
         begin

           Stream := cds_Mat_Restaurar.CreateBlobStream(cds_Mat_Restaurarfoto4, bmread);
           try
             if extensao = '.BMP' then
                begin
                  figura := tbitmap.Create;
                  try
                    figura.LoadFromStream(stream);
                    imgfoto4.Picture.Assign(figura)
                  finally
         ...
         begin
           ...
           if not directoryexists(temppath) then
              ...
              //endi
           //endi
           temppath := temppath + '\'+cds_Mat_Restaurarfoto4.AsString;

           //Grava bloco no arquivo temporário
           cds_Mat_Restaurarfoto4.SaveToFile(temppath);
...
procedure TfrmPesqMat_Rest.SpeedButton3Click(Sender: TObject);
begin
  if opdabrirfig.Execute then
     begin
       frmdados.cds_Mat_Restaurar.Edit;
       frmdados.cds_Mat_Restaurarfoto2.LoadFromFile(opdabrirfig.FileName);
       frmdados.cds_Mat_Restaurarfilename2.AsString := extractfilename(opdabrirfig.FileName);
       mostrararq_ext2;
     end;
  //endi
...
procedure TfrmPesqMat_Rest.SpeedButton5Click(Sender: TObject);
begin
  if opdabrirfig.Execute then
     begin
       frmdados.cds_Mat_Restaurar.Edit;
       frmdados.cds_Mat_Restaurarfoto3.LoadFromFile(opdabrirfig.FileName);
       frmdados.cds_Mat_Restaurarfilename3.AsString := extractfilename(opdabrirfig.FileName);
       mostrararq_ext3;
     end;
  //endi
...
procedure TfrmPesqMat_Rest.SpeedButton7Click(Sender: TObject);
begin
  if opdabrirfig.Execute then
     begin
       frmdados.cds_Mat_Restaurar.Edit;
       frmdados.cds_Mat_Restaurarfoto4.LoadFromFile(opdabrirfig.FileName);
       frmdados.cds_Mat_Restaurarfilename4.AsString := extractfilename(opdabrirfig.FileName);
       mostrararq_ext4;
     end;
  //endi
...
procedure TfrmPesqMat_Rest.SpeedButton4Click(Sender: TObject);
begin
  frmdados.Cds_Mat_Restaurar.Edit;
  frmdados.cds_Mat_Restaurarfoto2.AsVariant := '';
  mostrararq_ext2;

end;

procedure TfrmPesqMat_Rest.SpeedButton6Click(Sender: TObject);
begin
  frmdados.Cds_Mat_Restaurar.Edit;
  frmdados.cds_Mat_Restaurarfoto3.AsVariant := '';
  mostrararq_ext3;

end;

procedure TfrmPesqMat_Rest.SpeedButton8Click(Sender: TObject);
begin
  frmdados.Cds_Mat_Restaurar.Edit;
  frmdados.cds_Mat_Restaurarfoto4.AsVariant := '';
  mostrararq_ext4;
