...
uses uGeral, uPrincipal, uDados, urelvii ;

{$R *.DFM}

procedure TfrmMRvii.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelvii := tfrmrelvii.Create(self);
     frmrelvii.relatorio.Preview;
     frmrelvii.Free;
   end;
//endi
...
procedure TfrmMRvii.FormShow(Sender: TObject);
begin
...
procedure TfrmMRvii.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRvii.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRvii.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRvii.FormClose(Sender: TObject; var Action: TCloseAction);
begin
