...
type
  TfrmPesqEst = class(TForm)
    ...
    procedure dbgEstEnter(Sender: TObject);
    ...
    procedure dbgEstExit(Sender: TObject);
    ...
    procedure dbgEstKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses uGeral, uPrincipal, uDados, uMRufest;
...
procedure TfrmPesqEst.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqEst.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqEst.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqEst.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqEst.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmPesqEst.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqEst.dbgEstKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
   if frmdados.Cds_Estados.State in [dsinsert, dsedit] then
      frmdados.Cds_Estados.Post;
   //endi
//endi
end;

procedure TfrmPesqEst.dbgEstEnter(Sender: TObject);
begin
tabela := 'Estados';
end;

procedure TfrmPesqEst.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqEst.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqEst.spdfiltrarClick(Sender: TObject);
var
...
begin
  ...
  with frmdados do
    begin
      ...
      cds_estadosmunic.DefaultExpression := quotedstr('F');
    end;
  //end;
  ...
  try
    with frmdados.Dbx_Estados do
       begin
         active := false;
         sql.Clear;
         if sCompo <> '' then
            sql.Add('select * from Estados where '+sCompo+' order by '+sClassificar)
         else
            sql.Add('select * from Estados order by '+sClassificar);
         //endi
         active := true;
            ...
            begin
              if sDesc <> '' then
                 begin
                   active := false;
                   sql.Clear;
                   sql.Add('select * from Estados where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by '+sClassificar);
                   active := true;
                 end;
              //endi
            end;
...
procedure TfrmPesqEst.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqEst.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   frmdados.cds_Estados.Locate(frmpesqest.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure TfrmPesqEst.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqEst.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqEst.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqEst.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
if key = #13 then
   begin
     ...
     if edicodigoi.Text <> '' then
        if not frmdados.cds_Estados.Locate('codigo',edicodigoi.text,[])then
...
procedure TfrmPesqEst.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
procedure TfrmPesqEst.dbgEstExit(Sender: TObject);
begin
  if frmdados.Cds_Estados.State in [dsinsert, dsedit] then
     frmdados.Cds_Estados.Post;
  //endi
...
procedure TfrmPesqEst.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqEst.spdIncluir171Click(Sender: TObject);
begin
tabela := 'Estados';
dbgEst.SetFocus;
...
procedure TfrmPesqEst.spdantClick(Sender: TObject);
begin
  frmdados.Cds_Estados.Prior;

end;

procedure TfrmPesqEst.spdproxClick(Sender: TObject);
begin
frmdados.Cds_Estados.Next;
end;

procedure TfrmPesqEst.spdExcluir173Click(Sender: TObject);
begin
tabela := 'Estados';
...
procedure TfrmPesqEst.dbgEstKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmPesqEst.spdrelatorios176Click(Sender: TObject);
begin
    frmMRufest:=tfrmMRufest.create(self);
    frmMRufest.ShowModal;
    frmMRufest.Free;
end;

procedure TfrmPesqEst.sPdGravarClick(Sender: TObject);
begin
   if frmDados.Cds_Estados.State in [dsedit,dsinsert] then
       begin
         frmDados.Cds_Estados.Post;
       end
    //endif
