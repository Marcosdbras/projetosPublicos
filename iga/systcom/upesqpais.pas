...
type
  TfrmPesqPais = class(TForm)
    ...
    procedure dbgPaisKeyPress(Sender: TObject; var Key: Char);
    procedure dbgPaisEnter(Sender: TObject);
    ...
    procedure dbgPaisExit(Sender: TObject);
    ...
    procedure dbgPaisKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
uses uGeral, uPrincipal, uDados ,uMRpais;

{$R *.DFM}

procedure TfrmPesqPais.FormShow(Sender: TObject);
var
   vardir: String;
begin
...
procedure TfrmPesqPais.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqPais.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqPais.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqPais.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqPais.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmPesqPais.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqPais.dbgPaisKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
   if frmdados.Cds_pais.State in [dsinsert, dsedit] then
      frmdados.Cds_pais.Post;
   //endi
//endi
end;

procedure TfrmPesqPais.dbgPaisEnter(Sender: TObject);
begin
tabela := 'Pais';
end;

procedure TfrmPesqPais.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqPais.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqPais.spdfiltrarClick(Sender: TObject);
var
...
begin
  ...
  try
    with frmdados.Dbx_pais do
       begin
         active := false;
         sql.Clear;
         if sCompo <> '' then
            sql.Add('select * from pais where '+sCompo+' order by '+sClassificar)
         else
            sql.Add('select * from pais order by '+sClassificar);
         //endi
         active := true;
            ...
            begin
              if sDesc <> '' then
                 begin
                   active := false;
                   sql.Clear;
                   sql.Add('select * from pais where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by '+sClassificar);
                   active := true;
                 end;
              //endi
            end;
...
procedure TfrmPesqPais.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqPais.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   frmdados.cds_pais.Locate(frmpesqpais.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure TfrmPesqPais.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqPais.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqPais.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqPais.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
if key = #13 then
   begin
     ...
     if edicodigoi.Text <> '' then
        if not frmdados.cds_pais.Locate('codigo',edicodigoi.text,[])then
...
procedure TfrmPesqPais.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
//if key = #13 then
//   begin
...
procedure TfrmPesqPais.dbgPaisExit(Sender: TObject);
begin
  if frmdados.Cds_pais.State in [dsinsert, dsedit] then
     frmdados.Cds_pais.Post;
  //endi
...
procedure TfrmPesqPais.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqPais.spdIncluir171Click(Sender: TObject);
begin
tabela := 'Pais';
dbgpais.SetFocus;
frmdados.Cds_pais.Append;
end;

procedure TfrmPesqPais.spdantClick(Sender: TObject);
begin
  frmdados.Cds_pais.Prior;

end;

procedure TfrmPesqPais.spdproxClick(Sender: TObject);
begin
frmdados.Cds_pais.Next;
end;

procedure TfrmPesqPais.spdExcluir173Click(Sender: TObject);
begin
tabela := 'Pais';
if application.MessageBox('Deseja Realmente Excluir Esse Registro?','Atenção',mb_YesNo + mb_DefButton2) = idYes then
   begin

     frmdados.Cds_pais.Delete;
...
procedure TfrmPesqPais.dbgPaisKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmPesqPais.SpeedButton1Click(Sender: TObject);
begin
    frmMRpais := tfrmmrpais.create(self);
    frmMRpaiS.Showmodal;
    frmMRpais.Free;
end;
