...
var
  frmMRtpcli: TfrmMRtpcli;

implementation

uses uGeral, uPrincipal, uDados, ureltpcli ;

{$R *.DFM}

procedure TfrmMRtpcli.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmreltpcli := tfrmreltpcli.Create(self);
     frmreltpcli.relatorio.Preview;
     frmreltpcli.Free;
   end;
//endi
...
procedure TfrmMRtpcli.FormShow(Sender: TObject);
begin
...
procedure TfrmMRtpcli.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRtpcli.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRtpcli.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRtpcli.FormClose(Sender: TObject; var Action: TCloseAction);
begin
