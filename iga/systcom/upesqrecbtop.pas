...
uses
  ...
  Grids, DBGrids, SqlExpr, ImgList, jpeg, Menus;

type
  Tfrmpesqrecbtop = class(TForm)
    pnlsuperior: TPanel;
    ...
    procedure cbxnfunExit(Sender: TObject);
...
var
  frmpesqrecbtop: Tfrmpesqrecbtop;

implementation

uses uGeral, uDados, uMRRECp, urelrecbtop, urelcomp;

{$R *.DFM}

procedure Tfrmpesqrecbtop.FormShow(Sender: TObject);
var
   vardir: String;
begin
   ...
   cbxNomefun.ListSource := frmDados.Dst_Funcionarios;   //  vai ler no
   cbxNomefun.Keyfield   := 'codigo'; // o campo
   cbxNomefun.DataSource := frmdados.Dts_Temp;  // vai gravar no
   cbxNomefun.DataField  :='cod2';  // no campo
...
procedure Tfrmpesqrecbtop.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure Tfrmpesqrecbtop.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure Tfrmpesqrecbtop.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure Tfrmpesqrecbtop.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure Tfrmpesqrecbtop.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  frmPesqRecbtop:=nil;
  form_ativo := '';
...
procedure Tfrmpesqrecbtop.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure Tfrmpesqrecbtop.dbgrecbtoKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
...
procedure Tfrmpesqrecbtop.dbgrecbtoEnter(Sender: TObject);
begin
tabela := 'Vencimentos';
end;

procedure Tfrmpesqrecbtop.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure Tfrmpesqrecbtop.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure Tfrmpesqrecbtop.spdfiltrarClick(Sender: TObject);
var
  ...
  iccli, iccla, icsitRec, ictipopgto, ictipoop, icfun:integer;
begin
  ...
  icfun := frmdados.Cds_Temp.fieldbyname('cod2').asInteger;
     ...
     begin
       if (sCompo = '') then
          begin
            ...
            sTitRel := sTitRel + ' / Dentista: ' + cbxnomefun.Text;
          end
       else
          begin
...
procedure Tfrmpesqrecbtop.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure Tfrmpesqrecbtop.edidescChange(Sender: TObject);
begin
if (edidesc.Text <> '') and (sclassificar <> 'Dte') and (sclassificar <> 'Dtv') and (sclassificar <> 'Dtl') and (sclassificar <> 'Dtb') then
   frmdados.cds_Vencto.Locate(frmpesqrecbtop.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure Tfrmpesqrecbtop.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure Tfrmpesqrecbtop.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure Tfrmpesqrecbtop.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure Tfrmpesqrecbtop.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
if key = #13 then
   begin
...
procedure Tfrmpesqrecbtop.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
if key = #13 then
   begin
...
procedure Tfrmpesqrecbtop.dbgrecbtoExit(Sender: TObject);
begin
  if frmdados.Cds_Vencto.State in [dsinsert, dsedit] then
     frmdados.Cds_Vencto.Post;
  //endi
...
procedure Tfrmpesqrecbtop.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure Tfrmpesqrecbtop.cbxclassificar2KeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
...
procedure Tfrmpesqrecbtop.cbxclassificar2KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure Tfrmpesqrecbtop.cbxclassificarClick(Sender: TObject);
begin
habdedidesc;
spdfiltrar.Click;
end;

procedure tfrmpesqrecbtop.habdedidesc;
begin
...
procedure Tfrmpesqrecbtop.cbxnomecliClick(Sender: TObject);
begin
tabela := 'Temp';
end;

procedure Tfrmpesqrecbtop.cbxnomecliEnter(Sender: TObject);
begin
tabela := 'Temp';
end;

procedure Tfrmpesqrecbtop.cbxnomeclaClick(Sender: TObject);
begin
tabela := 'temp';
end;

procedure Tfrmpesqrecbtop.cbxnomeclaEnter(Sender: TObject);
begin
tabela := 'Temp';
end;

procedure Tfrmpesqrecbtop.cbxnomecliKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmpesqrecbtop.cbxnomesitrecClick(Sender: TObject);
begin
tabela := 'Temp';
end;

procedure Tfrmpesqrecbtop.cbxnomesitrecEnter(Sender: TObject);
begin
tabela := 'Temp';
end;

procedure Tfrmpesqrecbtop.cbxnomeclaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
//if key = vk_delete then
//   frmdados.Cds_temp.edit;
...
procedure Tfrmpesqrecbtop.cbxnomesitrecKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
//if key = vk_delete then
//   frmdados.Cds_temp.edit;
...
procedure Tfrmpesqrecbtop.spdIncluir171Click(Sender: TObject);
begin
if frmdados.Cds_Vencto.State in [dsinsert, dsedit] then
   begin
     frmdados.Cds_Vencto.CancelUpdates;
...
procedure Tfrmpesqrecbtop.spdAlterar172Click(Sender: TObject);
begin
sOpcao := 'A';
...
procedure Tfrmpesqrecbtop.spdExcluir173Click(Sender: TObject);
begin
sOpcao := 'E';
spdgravar.Visible := true;
spdcancelar2310.Left := iSpdcancelar2310a;
...
procedure Tfrmpesqrecbtop.spdConsultar174Click(Sender: TObject);
begin
sOpcao := 'C';
...
procedure Tfrmpesqrecbtop.spdmenurelClick(Sender: TObject);
begin
//frmMRRECp := tfrmMRRECp.Create(self);
//frmMRRECp.showmodal;
//frmMRRECp.Free;

frmrelrecbtop := tfrmrelrecbtop.Create(self);
frmrelrecbtop.relatorio.Preview;
frmrelrecbtop.Free;


end;

procedure Tfrmpesqrecbtop.sPdGravarClick(Sender: TObject);
var
 J : TJPEGImage;
 figura : tbitmap;
 S : TMemoryStream;
...
procedure Tfrmpesqrecbtop.spdcancelar2310Click(Sender: TObject);
begin
...
procedure Tfrmpesqrecbtop.AlternaF;
begin
...
procedure Tfrmpesqrecbtop.AlternaL;
begin
...
procedure Tfrmpesqrecbtop.bloqueiacampos;
begin
pnlficha.Enabled         := false;
end;

procedure Tfrmpesqrecbtop.desbloqueiacampos;
begin
...
procedure tfrmpesqrecbtop.bloqueiabotoes;
begin
...
procedure tfrmpesqrecbtop.desbloqueiabotoes;
begin
...
procedure Tfrmpesqrecbtop.spdantClick(Sender: TObject);
begin
tabela := 'Vencimentos';
if frmdados.Cds_Vencto.State in [dsinsert, dsedit] then
   begin
...
procedure Tfrmpesqrecbtop.spdproxClick(Sender: TObject);
begin
tabela := 'Vencimentos';
if frmdados.Cds_Vencto.State in [dsinsert, dsedit] then
   begin
...
procedure Tfrmpesqrecbtop.edidescricaoKeyPress(Sender: TObject; var Key: Char);
begin
if (key = #13) then
   begin
     key := #0;
...
procedure Tfrmpesqrecbtop.ediccli_fKeyPress(Sender: TObject; var Key: Char);
begin
if (key = #13) then
   begin
     key := #0;
...
procedure Tfrmpesqrecbtop.cbxnfunExit(Sender: TObject);
begin
  //frmdados.Cds_Vencto.Edit;
  //edivcom.Text := formatfloat('0.00',frmdados.Cds_vencto.fieldbyname('valliq').asfloat*strtofloat(tirapontos(edipcom.Text))/100);
end;

procedure Tfrmpesqrecbtop.edipcomExit(Sender: TObject);
begin
  frmdados.Cds_Vencto.Edit;
  frmdados.Cds_Vencto.FieldByName('vcom').asfloat := frmdados.Cds_vencto.fieldbyname('valliq').asfloat* strtofloat(tirapontos(edipcom.Text))/100;
end;

procedure Tfrmpesqrecbtop.edivcomExit(Sender: TObject);
begin
  frmdados.Cds_Vencto.Edit;
  frmdados.Cds_Vencto.FieldByName('pcom').asfloat := strtofloat(tirapontos(edivcom.text))/strtofloat(tirapontos(edivalliq.text))*100;
end;

procedure Tfrmpesqrecbtop.Completo1Click(Sender: TObject);
begin
frmrelrecbtop := tfrmrelrecbtop.Create(self);
frmrelrecbtop.relatorio.Preview;
frmrelrecbtop.Free;
...
procedure Tfrmpesqrecbtop.Comisso1Click(Sender: TObject);
begin
frmrelcomp := tfrmrelcomp.Create(self);
frmrelcomp.relatorio.Preview;
frmrelcomp.Free;
