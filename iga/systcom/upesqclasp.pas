...
type
  TfrmPesqclasp = class(TForm)
    ...
    procedure dbgclassesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure tbsclaspriShow(Sender: TObject);
...
procedure TfrmPesqclasp.FormShow(Sender: TObject);
var
   vardir: String;
begin
...
procedure TfrmPesqclasp.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqclasp.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqclasp.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqclasp.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqclasp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  frmPesqclasp:=nil;
  form_ativo := '';
...
procedure TfrmPesqclasp.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqclasp.dbgclassesKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
...
procedure TfrmPesqclasp.dbgclassesEnter(Sender: TObject);
begin
tabela := 'Classe';
end;

procedure TfrmPesqclasp.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqclasp.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqclasp.spdfiltrarClick(Sender: TObject);
var
...
begin
  ...
  with frmdados do
    begin
      if pclres.ActivePageIndex = 2 then
         Cds_Classe.Active := false;
      //endi
      if pclres.ActivePageIndex = 1 then
         Cds_Classe.Active := false;
      //endi
      if pclres.ActivePageIndex = 0 then
         Cds_Classep.Active := false;
      //endi
    end;
  //end;
            ...
            begin
              if pclres.ActivePageIndex = 2 then
                 begin
                   ...
                   frmdados.Dbx_Classe.sql.Clear;
                   frmdados.Dbx_Classe.sql.Add('select * from Classe where '+sCompo+' order by cclap, '+sClassificar);
                   frmdados.Dbx_Classe.active := true;
                 end;
              //endi
              if pclres.ActivePageIndex = 1 then
                 begin
              ...
              if pclres.ActivePageIndex = 0 then
                 begin
                   frmdados.Dbx_Classep.active := false;
                   frmdados.Dbx_Classep.sql.Clear;
                   frmdados.Dbx_Classep.sql.Add('select * from Classep where '+sCompo+' order by '+sClassificar);
                   frmdados.Dbx_Classep.active := true;
                 end;
              //endi
            end
         else
            begin
              if pclres.ActivePageIndex = 2 then
                 begin
                   frmdados.Dbx_Classe.active := false;
                   frmdados.Dbx_Classe.sql.Clear;
                   frmdados.Dbx_Classe.sql.Add('select * from Classe order by cclap, '+sClassificar);
                   frmdados.Dbx_Classe.active := true;
                 end;
              //endi
              if pclres.ActivePageIndex = 1 then
              ...
              if pclres.ActivePageIndex = 0 then
                 begin
                   frmdados.Dbx_Classep.active := false;
                   frmdados.Dbx_Classep.sql.Clear;
                   frmdados.Dbx_Classep.sql.Add('select * from Classep order by '+sClassificar);
                   frmdados.Dbx_Classep.active := true;
                 end;
              //endi
            end;
         ...
         //   begin
              ...
              if sDesc <> '' then
                 begin
                   ...
                   if pclres.ActivePageIndex = 2 then
                      begin
                        if frmdados.Dbx_Classe.RecordCount = 0 then
                           begin
                             frmdados.Dbx_Classe.Active := false;
                             frmdados.Dbx_Classe.SQL.Clear;
                             frmdados.Dbx_Classe.sql.Add('select * from Classe where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by cclap, '+sClassificar);
                             frmdados.Dbx_Classe.Active := true;
                           end;
                        //endi
                   ...
                   if pclres.ActivePageIndex = 0 then
                      begin
                        if frmdados.Dbx_Classep.RecordCount = 0 then
                           begin
                             frmdados.Dbx_Classep.Active := false;
                             frmdados.Dbx_Classep.SQL.Clear;
                             frmdados.Dbx_Classep.sql.Add('select * from Classep where '+sClassificar+' like '+quotedstr('%'+sDesc+'%')+'order by '+sClassificar);
                             frmdados.Dbx_Classep.Active := true;
                           end;
                        //endi
                      end;
...
procedure TfrmPesqclasp.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqclasp.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   begin
     if pclres.ActivePageIndex = 2 then
        frmdados.cds_Classe.Locate(frmpesqclasp.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
     //endi
     if pclres.ActivePageIndex = 1 then
        frmdados.cds_Classe.Locate(frmpesqclasp.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
     //endi
     if pclres.ActivePageIndex = 0 then
        frmdados.cds_Classep.Locate(frmpesqclasp.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
     //endi
   end;
//endi
...
procedure TfrmPesqclasp.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqclasp.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqclasp.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqclasp.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
if key = #13 then
   begin
...
procedure TfrmPesqclasp.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
if key = #13 then
   begin
...
procedure TfrmPesqclasp.dbgclassesExit(Sender: TObject);
begin
  if frmdados.Cds_Classe.State in [dsinsert, dsedit] then
     frmdados.Cds_Classe.Post;
  //endi
...
procedure TfrmPesqclasp.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqclasp.res_ano;
var
  iano, imes, idia:word;
begin
...
procedure TfrmPesqclasp.tbsresreaShow(Sender: TObject);
var
...
procedure TfrmPesqclasp.tbslistaShow(Sender: TObject);
begin
tabela := 'Classe';
//spdlimpar.Click;
...
procedure TfrmPesqclasp.titcol;
var
...
procedure TfrmPesqclasp.linhadetpago(ilinha:integer);
var
...
procedure TfrmPesqclasp.subtotpago(ilinha:integer);
var
  icoluna, icontador:integer;
  imes, idia, iano:word;
begin
...
procedure TfrmPesqclasp.totpagomes(ilinha:integer);
var
  icoluna, icontador:integer;
  imes, idia, iano:word;
begin
...
procedure tfrmPesqclasp.limparsubtot;
begin
  fsubtotjan := 0;
  fsubtotfev := 0;
  fsubtotmar := 0;
...
procedure tfrmPesqclasp.limpartotmes;
begin
  ftotjan := 0;
  ftotfev := 0;
  ftotmar := 0;
...
procedure TfrmPesqclasp.dbgclassesKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmPesqclasp.tbsclaspriShow(Sender: TObject);
begin
tabela := 'Classep';
//spdlimpar.Click;

if frmdados.Cds_Classep.Active then
   frmdados.Cds_Classep.First;
//endi


end;

procedure TfrmPesqclasp.dbgclassepEnter(Sender: TObject);
begin
tabela := 'Classep';
end;
