...
var
  frmMRoimp: TfrmMRoimp;

implementation

uses uGeral, uPrincipal, uDados, ureloimp ;

{$R *.DFM}

procedure TfrmMRoimp.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmreloimp := tfrmreloimp.Create(self);
     frmreloimp.relatorio.Preview;
     frmreloimp.Free;
   end;
//endi
...
procedure TfrmMRoimp.FormShow(Sender: TObject);
begin
...
procedure TfrmMRoimp.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRoimp.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRoimp.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRoimp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
