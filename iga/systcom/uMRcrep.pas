...
var
  frmMRcrep: TfrmMRcrep;

implementation

uses uGeral, uPrincipal, uDados, urelcrep , urelcreps ;

{$R *.DFM}

procedure TfrmMRcrep.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelcrep := tfrmrelcrep.Create(self);
     frmrelcrep.relatorio.Preview;
     frmrelcrep.Free;
   end;
//endi
...
if rgbtiporel.ItemIndex = 1 then
   begin
     frmrelcreps := tfrmrelcreps.create(self);
     frmrelcreps.relatorio.preview;
     frmrelcreps.free;
   end;
//endi
...
procedure TfrmMRcrep.FormShow(Sender: TObject);
begin
...
procedure TfrmMRcrep.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRcrep.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRcrep.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRcrep.FormClose(Sender: TObject; var Action: TCloseAction);
begin
