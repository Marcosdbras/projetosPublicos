...
var
  frmMRSprod: TfrmMRSprod;

implementation

uses uGeral, uPrincipal, uDados, urelSprod ;

{$R *.DFM}

procedure TfrmMRSprod.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelSprod := tfrmrelSprod.Create(self);
     frmrelSprod.relatorio.Preview;
     frmrelSprod.Free;
   end;
//endi
...
procedure TfrmMRSprod.FormShow(Sender: TObject);
begin
...
procedure TfrmMRSprod.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRSprod.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRSprod.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRSprod.FormClose(Sender: TObject; var Action: TCloseAction);
begin
