...
var
  frmPesqforn: TfrmPesqforn;

implementation

uses uGeral, uPrincipal, uDados , umrforn;

{$R *.DFM}

procedure TfrmPesqforn.FormShow(Sender: TObject);
var
   vardir: String;
begin
...
procedure TfrmPesqforn.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqforn.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqforn.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmPesqforn.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmPesqforn.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TfrmPesqforn.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmPesqforn.dbgFornecedoresKeyPress(Sender: TObject; var Key: Char);
begin
if key = #13 then
...
procedure TfrmPesqforn.dbgFornecedoresEnter(Sender: TObject);
begin
tabela := 'Fornecedores';
end;

procedure TfrmPesqforn.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqforn.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmPesqforn.spdfiltrarClick(Sender: TObject);
var
...
procedure TfrmPesqforn.spdlimparClick(Sender: TObject);
begin
  edicodigoi.Text := '';
  edicodigof.Text := '';
...
procedure TfrmPesqforn.edidescChange(Sender: TObject);
begin
if edidesc.Text <> '' then
   frmdados.cds_Fornecedores.Locate(frmpesqforn.sClassificar,edidesc.text,[loPartialKey,loCaseInsensitive]);
//endi
end;

procedure TfrmPesqforn.cbxclassificarKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
     begin
...
procedure TfrmPesqforn.cbxclassificarKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmPesqforn.edidescKeyPress(Sender: TObject; var Key: Char);
begin
edicodigoi.Text := '';
edicodigof.Text := '';
if key = #13 then
...
procedure TfrmPesqforn.edicodigoiKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
if key = #13 then
   begin
...
procedure TfrmPesqforn.edicodigofKeyPress(Sender: TObject; var Key: Char);
begin
edidesc.Text := '';
if key = #13 then
   begin
...
procedure TfrmPesqforn.dbgFornecedoresExit(Sender: TObject);
begin
  if frmdados.Cds_Fornecedores.State in [dsinsert, dsedit] then
     frmdados.Cds_Fornecedores.Post;
  //endi
...
procedure TfrmPesqforn.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
if key = 27 then
   close;
...
procedure TfrmPesqforn.dbgFornecedoresKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmPesqforn.SpeedButton1Click(Sender: TObject);
begin
  frmmrforn:=tfrmmrforn.create(self);
  frmmrforn.showmodal;
  frmmrforn.free;
end;
