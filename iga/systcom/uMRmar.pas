...
uses uGeral, uPrincipal, uDados, urelmar ;

{$R *.DFM}

procedure TfrmMRmar.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelmar := tfrmrelmar.Create(self);
     frmrelmar.relatorio.Preview;
     frmrelmar.Free;
   end;
//endi
...
procedure TfrmMRmar.FormShow(Sender: TObject);
begin
...
procedure TfrmMRmar.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRmar.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRmar.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRmar.FormClose(Sender: TObject; var Action: TCloseAction);
begin
