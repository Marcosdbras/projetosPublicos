...
procedure TfrmCaixaApap.ediOSExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmCaixaApap.ediosefExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmCaixaApap.ediparcExit(Sender: TObject);
var
   valor : integer;
begin
...
procedure TfrmCaixaApap.edimaxparcExit(Sender: TObject);
var
   valor : integer;
begin
end;

procedure TfrmCaixaApap.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  frmCaixaApap:=nil;
  form_ativo := '';
...
procedure TfrmCaixaApap.mmoobsKeyPress(Sender: TObject; var Key: Char);
begin
   if (key = #13) then SelectNext(ActiveControl,True,True);
end;

procedure TfrmCaixaApap.spdconfirmaClick(Sender: TObject);
begin
close;
end;

procedure TfrmCaixaApap.spdcancelaClick(Sender: TObject);
begin
close;
end;

procedure TfrmCaixaApap.dbgrecKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmCaixaApap.FormShow(Sender: TObject);
...
procedure TfrmCaixaApap.spdfiltrarClick(Sender: TObject);
  var pCodSit, pCodfunreb, pCodfuncon, topo, A:Integer;
...
if iNum = 1 then
  begin
    ...
    for x := 0 to frmcaixaapap.ControlCount - 1 do
      begin
        if frmcaixaapap.Controls[x].Tag = 1 then
           begin
             frmcaixaapap.Controls[x].Visible := false;
           end;
        //endi
      end;
    //endf
...
with frmdados do
  begin
    ...
    if cbxtipo.ItemIndex = 0 then
       dbx_svenda.SQL.Add('Select * from svenda where ('+'datafec >= '+quotedstr(sDataI)+') and (datafec <= '+quotedstr(sDataF)+') and (lancado = '+Quotedstr('T')+')');
    //endi
    if cbxtipo.ItemIndex = 1 then
    ...
    //endi
    if cbxtipo.ItemIndex = 2 then
       dbx_svenda.SQL.Add('Select * from svenda where ('+'datafec >= '+quotedstr(sDataI)+') and (datafec <= '+quotedstr(sDataF)+') and (lancado = '+Quotedstr('T')+') and (nco is null) and (nos is null)');
    //endi
    if cbxtipo.ItemIndex = 3 then
       dbx_svenda.SQL.Add('Select * from svenda where ('+'datafec >= '+quotedstr(sDataI)+') and (datafec <= '+quotedstr(sDataF)+') and (lancado = '+Quotedstr('T')+') and (coalesce (nos,0) > 0)');
    //endi
...
Procedure TfrmCaixaApap.CriaSeries;
begin
...
procedure TfrmCaixaApap.MontaSerie(SR: TPieSeries; LR: TStrings; RR:TStrings);
var
  K: Integer;
begin
  SR.Clear;
...
procedure TfrmCaixaApap.spdlimparClick(Sender: TObject);
var
  x:integer;
...
procedure TfrmCaixaApap.cbxtipoKeyPress(Sender: TObject; var Key: Char);
begin
key := #0;
end;

procedure TfrmCaixaApap.cbxtipoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
key := 0;
end;

procedure TfrmCaixaApap.spdsangriaClick(Sender: TObject);
begin
...
procedure TfrmCaixaApap.spdtrocoClick(Sender: TObject);
begin
...
procedure TfrmCaixaApap.spdimpcaixaClick(Sender: TObject);
begin
...
procedure TfrmCaixaApap.spdgraficoClick(Sender: TObject);
begin
...
procedure TfrmCaixaApap.FormCreate(Sender: TObject);
begin
iNum := 0;
end;
