...
var
  frmMRtrans: TfrmMRtrans;

implementation

uses uGeral, uPrincipal, uDados, ureltrans , ureltranss;

{$R *.DFM}

procedure TfrmMRtrans.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmreltrans := tfrmreltrans.Create(self);
     frmreltrans.relatorio.Preview;
     frmreltrans.Free;
   end;
//endi
...
if rgbtiporel.ItemIndex = 1 then
   begin
   frmreltranss:=tfrmreltranss.Create(self);
   frmreltranss.relatorio.Preview;
   frmreltranss.Free;
   end;
//endi
...
procedure TfrmMRtrans.FormShow(Sender: TObject);
begin
...
procedure TfrmMRtrans.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRtrans.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRtrans.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRtrans.FormClose(Sender: TObject; var Action: TCloseAction);
begin
