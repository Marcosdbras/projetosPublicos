...
uses uGeral, uPrincipal, uDados, urelani, urelcfd ,urelcfs ;

{$R *.DFM}

procedure TfrmMRcf.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelcfd := tfrmrelcfd.create(self);
     frmrelcfd.relatorio.preview;
     frmrelcfd.free;
   end;
//endi
...
if rgbtiporel.ItemIndex = 1 then
   begin
     frmrelcfs := tfrmrelcfs.create(self);
     frmrelcfs.relatorio.preview;
     frmrelcfs.free;
   end;
//endi
...
procedure TfrmMRcf.FormShow(Sender: TObject);
begin
...
procedure TfrmMRcf.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRcf.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRcf.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRcf.FormClose(Sender: TObject; var Action: TCloseAction);
begin
