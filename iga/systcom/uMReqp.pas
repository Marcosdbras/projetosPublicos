...
uses uGeral, uPrincipal, uDados, ureleqp ;

{$R *.DFM}

procedure TfrmMReqp.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmreleqp := tfrmreleqp.Create(self);
     frmreleqp.relatorio.Preview;
     frmreleqp.Free;
   end;
//endi
...
procedure TfrmMReqp.FormShow(Sender: TObject);
begin
...
procedure TfrmMReqp.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMReqp.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMReqp.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMReqp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
