...
uses uGeral, uPrincipal, uDados, urelgru ;

{$R *.DFM}

procedure TfrmMRgru.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelgru := tfrmrelgru.Create(self);
     frmrelgru.relatorio.Preview;
     frmrelgru.Free;
   end;
//endi
...
procedure TfrmMRgru.FormShow(Sender: TObject);
begin
...
procedure TfrmMRgru.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRgru.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRgru.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRgru.FormClose(Sender: TObject; var Action: TCloseAction);
begin
