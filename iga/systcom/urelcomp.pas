...
var
  frmrelcomp: Tfrmrelcomp;

implementation
   uses udados, ugeral, upesqrecbtop;
{$R *.dfm}

procedure Tfrmrelcomp.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
ipag := ipag + 1;
...
procedure Tfrmrelcomp.relatorioBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
ipag := 0;
ftotal := 0;
...
procedure Tfrmrelcomp.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
...
procedure Tfrmrelcomp.QRBand2BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  ftotal := ftotal + frmdados.Cds_Vencto.fieldbyname('vcom').asfloat;
end;

procedure Tfrmrelcomp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:=caFree;
  frmrelcomp:=nil;
