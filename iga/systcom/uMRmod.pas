...
uses uGeral, uPrincipal, uDados, urelmod ;

{$R *.DFM}

procedure TfrmMRmod.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelmod := tfrmrelmod.Create(self);
     frmrelmod.relatorio.Preview;
     frmrelmod.Free;
   end;
//endi
...
procedure TfrmMRmod.FormShow(Sender: TObject);
begin
...
procedure TfrmMRmod.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRmod.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRmod.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRmod.FormClose(Sender: TObject; var Action: TCloseAction);
begin
