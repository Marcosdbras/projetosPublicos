...
uses uGeral, uPrincipal, uDados, urelund ;

{$R *.DFM}

procedure TfrmMRund.sPdGravarClick(Sender: TObject);
  var
    vardir:String;
begin
...
if rgbtiporel.ItemIndex = 0 then
   begin
     frmrelund := tfrmrelund.Create(self);
     frmrelund.relatorio.Preview;
     frmrelund.Free;
   end;
//endi
...
procedure TfrmMRund.FormShow(Sender: TObject);
begin
...
procedure TfrmMRund.rgbtiporelClick(Sender: TObject);
begin
spdgravar.Enabled := true;

end;

procedure TfrmMRund.spdSairClick(Sender: TObject);
begin
close;
end;

procedure TfrmMRund.Timer1Timer(Sender: TObject);
begin
...
procedure TfrmMRund.FormClose(Sender: TObject; var Action: TCloseAction);
begin
