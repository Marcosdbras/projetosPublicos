...
uses
  ...
  Dialogs, StdCtrls, Mask, Registry, ExtCtrls, DB, MemDS, DBAccess, IBC,
  RzPrgres, ComCtrls, IniFiles, Menus, XPMan, rxToolEdit;
...
type
  TfrmLista_Vendas_Periodo = class(TForm)
    ...
    procedure Bloco_GClick(Sender: TObject);    
...
var
  ...
  // variaveis do sintegra - DataSPED
...
begin
 if retorno = -1 then
 // showmessage('Erro na chamada da função! Verifique o erro em C:\DATASAC\paf\Log_Erro.txt')
 else
 if retorno = 0 then
...
begin
  rchSped.SelAttributes.Color := Cor;
  rchSped.SelAttributes.Size := 8;
  rchSped.Lines.Add(Log);
...
begin
 ...
 // bloco G
 Bloco_GClick(frmLista_Vendas_Periodo);
 // bloco h
...
// ABERTURA DO ARQUIVO DIGITAL E IDENTIFICAÇÃO DO CONTRIBUINTE
 ...
 TempInt := Registro0000('004',                           //Layout
 ...
 Trata_SPED_Int(sender, TempInt);
  ...
  try
   TempInt := Registro0005(sempresa_nome,        //Nome de fantasia associado:
                           somenteNumero(sempresa_cep),              //Código de Endereçamento Postal:
                           ...
                           somenteNumero(sempresa_telefone),              //Número do telefone:
                           somenteNumero(sempresa_fax),              //Número do fax:
...
procedure TfrmLista_Vendas_Periodo.Bloco_GClick(Sender: TObject);
var
 TempInt: Integer;
 sQtde, stotal : string;
begin
...
begin
  ...
  TempInt := RegistroH005(ed_data_fim.Text,           // Data do inventário
                          formatfloat('#,##0.00',query.fieldbyname('total').asFloat),                     // Valor total do estoque
  );
  ...
  begin
    ...
    begin
       ...
       begin
         ...
         stotal := FormatFloat('#,##0.00',(query.fieldbyname('qtde').asfloat * query.fieldbyname('precocusto').asfloat));
       end;
       ...
       TempInt := RegistroH010(
                               ...
                               formatfloat('#,##0.00',query.fieldbyname('precocusto').asFloat),                      // Valor unitário do item
...
procedure TfrmLista_Vendas_Periodo.FormCreate(Sender: TObject);
