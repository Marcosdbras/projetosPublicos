...
var
  ...
  // variaveis do sintegra - siscom 7
...
uses modulo_nfe, principal, funcoes, unECF, unSintegra, msg_Operador;
...
procedure TfrmSpedFiscal.Trata_SPED_Int(sender: tobject; retorno: integer);
begin
 if retorno = -1 then
 // showmessage('Erro na chamada da função! Verifique o erro em C:\siscom\paf\Log_Erro.txt')
 else
 if retorno = 0 then
...
begin
  ...
  begin
    ...
    frmMsg_operador.lb_msg.Caption := 'Aguarde! Gerando Arquivo de Vendas do Período...';
...
//      sMsg := sArquivoSintegra;
    ...
    begin
      sMensagem := sMensagem + #13 + #13
        + 'Arquivo do SINTEGRA não foi encontrado!'
    end;
...
procedure TfrmSpedFiscal.ed_data_iniKeyPress(Sender: TObject; var Key: Char);
begin
...
//           qrsaida_item.fieldbyname('outras_icms').value
     ...
     begin
          ...
          IF qrsaida_temp.RecordCount = 0 then
          begin
            texto := pansichar('A nota fiscal no.: '+QRADICIONA_ITEM.fieldbyname('numero').asstring+' possue dados incorretos! Verifique o modelo, o código do cliente e o número da nota!');
            application.messagebox(texto,'NF Emitida - SAI',mb_OK);
            abort;
          end;
...
procedure TfrmSpedFiscal.bt_sintegraClick(Sender: TObject);
var
 Err_Msg: string;
...
begin
  ...
  // registro 60R
            ...
            if sRegistro60R(Err_Msg, Qnt_Ok, Qnt_Erro) then
                 ...
                 begin
                      if Qnt_Ok > 0 then
                           Add_Log(inttostr(Qnt_Ok) + ' Registro(s) tipo 60R gerado(s) com sucesso.', clgreen);
                           Add_Log(inttostr(Qnt_Erro) + ' Registro(s) tipo 60R não gerado(s) por Erro' + #13 +
                           ' RLog de Erros: ' + Err_Msg, clred);
                 end;
 ...
 if sRegistro61(Err_Msg, Qnt_Ok, Qnt_Erro) then
  ...
  begin
   ...
   Add_Log(inttostr(Qnt_Erro) + ' Registro(s) tipo 61 não gerado(s) por Erro' + #13 +
     ' Log de Erros: ' + Err_Msg, clred);
  end;
          ...
          if sRegistro61R(Err_Msg, Qnt_Ok, Qnt_Erro) then
               ...
               begin
                    if Qnt_Ok > 0 then
                         ...
                         Add_Log(inttostr(Qnt_Erro) + ' Registro(s) tipo 61R não gerado(s) por Erro' + #13 +
                         ' RLog de Erros: ' + Err_Msg, clred);
               end;
     end;
...
procedure tfrmSpedFiscal.Add_Log(Log: string; Cor: TColor; Tamanho: byte);
begin
...
function tfrmSpedFiscal.Sintegra_Process: boolean;
begin
...
function tfrmSpedFiscal.Trata_SIntegra_Str(TempStr: string): boolean;
var
 sFile: TextFile;
begin
 Result := True;
 ...
 if TempStr[1] <> '-' then
  begin
   ...
   if not fileexists(sArquivoSintegra) then
    begin
     rewrite(sFile);
     Add_Log('Arquivo de saída criado: ' + sArquivoSintegra, clgreen);
    end;
...
function tfrmSpedFiscal.sRegistro10(var Err_Msg: string): boolean;
var
     TempStr: string;
begin
...
function tfrmSpedFiscal.sRegistro11(var Err_Msg: string): boolean;
var
     TempStr: string;
begin
     //Executa a seleção dos registros no banco de dados
     ...
     frmmodulo.qrfilial.FieldByName('numero').AsString,                             //Número
     ...
     if not Result then
          begin
               ...
               '    Número: ' + frmmodulo.qrfilial.FieldByName('numero').AsString + #13 +
...
function TfrmSpedFiscal.sRegistro50(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
...
begin
     ...
     //Geração de Registro50 quanto a Entrada  de Nota Fiscal Emitida
     //Executa a seleção dos registros no banco de dados
     with QrySintegra do
          begin
               Close;
...
function TfrmSpedFiscal.sRegistro54(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
 TempStr, num_nf: string;
begin
 Result := True;
...
//Registro60
function TfrmSpedFiscal.sRegistro60(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
                  ...
                  begin
                    ...
                    if not Trata_SIntegra_Str(TempStr) then
                      begin
                        ...
                        'Nº Serie.........: ' + query.FieldByName('nro_serie_eqp').AsString + #13 +
                        'Cód Produto......: ' + query.FieldByName('cd_produto').AsString + #13 +
                        ...
                        'Base de Cálc ICMS: ' + formatcurr('0.00', BASE_ICMS)+ #13 +
                        'Alíquota ICMS....: ' + formatcurr('0.00', ICMS)+ #13 +
...
//Registro60D - resumo diario
function TfrmSpedFiscal.sRegistro60D(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
 aliquota, TempStr: string;
 i: integer;
  BASE_ICMS, VALOR_ICMS, icms : real;
...
//               if RecordCount = 0 then
//                    showmessage('Não há itens de venda na ECF, verifique...');
                                 ...
                                 begin
                                   ...
                                   if not Trata_SIntegra_Str(TempStr) then
                                        begin
                                             ...
                                             'Nº Serie.........: ' + Fields.FieldByName('nro_serie_eqp').AsString + #13 +
                                             'Cód Produto......: ' + Fields.FieldByName('cd_produto').AsString + #13 +
...
//Registro60I - Itens
function TfrmSpedFiscal.sRegistro60I(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
 TempStr: string;
 i: integer;
 BASE_ICMS, VALOR_ICMS, icms : real;
...
function TfrmSpedFiscal.sRegistro60R(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
 TempStr: string;
 i: integer;
  BASE_ICMS, VALOR_ICMS, icms : real;
...
function TfrmSpedFiscal.sRegistro61(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
     TempStr, num_nf: string;
...
begin
  ...
  //Geração de Registro61 quanto a Saída de Nota Fiscal
  //Executa a seleção dos registros no banco de dados
        ...
        while QrySintegra.Fields.FieldByName('data').AsDateTime = vdata do begin
         ...
         if (QrySintegra.Fields.FieldByName('data').AsDateTime <> vdata) or (QrySintegra.Eof) then
           begin
             ...
             vcod_inicial := trim(copy(vcod_inicial, length(vcod_inicial) - 6, length(vcod_inicial)));        //utilizando apenas 6 últimos digitos da NF
             vcod_final   := trim(copy(vcod_final, length(vcod_final) - 6, length(vcod_final)));        //utilizando apenas 6 últimos digitos da NF
...
function TfrmSpedFiscal.sRegistro61R(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
 TempStr: string;
 i: integer;
  BASE_ICMS, VALOR_ICMS, icms : real;
...
function TfrmSpedFiscal.sRegistro75(var Err_Msg: string; var qnt_ok, qnt_erro: integer): boolean;
var
 TempStr: string;
 unidade : string;
begin
...
function TfrmSpedFiscal.sRegistro90: boolean;
var
 TempStr: string;
begin
...
function TfrmSpedFiscal.RoundNum(Valor:Extended;Decimais:Integer):Extended;
...
procedure TfrmSpedFiscal.Bloco_0_AberturaClick(Sender: TObject);
var
 TempInt: integer;
begin
...
// ABERTURA DO ARQUIVO DIGITAL E IDENTIFICAÇÃO DO CONTRIBUINTE
 ...
 TempInt := Registro0000('002',                           //Layout
                         ...
                         sEmpresa_CNPJ,                            //Número de inscrição do contribuinte:
                         '',                             //Número de inscrição do contribuinte:
                         sEmpresa_UF,                              //Sigla da unidade da federação:
                         somenteNumero(sempresa_ie),                              //Inscrição Estadual do contribuinte:
                         sempresa_ibge,                       //Código do município do domicílio fiscal:
 ...
 Trata_SPED_Int(sender, TempInt);
  ...
  try
   TempInt := Registro0005(sempresa_nome,        //Nome de fantasia associado:
                           sempresa_cep,              //Código de Endereçamento Postal:
                           sEmpresa_rua,              //Logradouro e endereço do imóvel:
                           sempresa_numero,                     //Número do imóvel:
                           sempresa_complemento,             //Dados complementares do endereço:
                           sempresa_bairro,                  //Bairro em que o imóvel está situado:
                           sempresa_telefone,              //Número do telefone:
                           sempresa_fax,              //Número do fax:
                           sempresa_email         //Endereço do correio eletrônico:
                           );
  ...
  TempInt := Registro0100(frmmodulo.qrfilial.fieldbyname('contador_nome').asstring,    //Nome do contabilista/escritório:
                          frmmodulo.qrfilial.fieldbyname('contador_cpf').asstring,                       //Número de inscrição no CPF:
                          frmmodulo.qrfilial.fieldbyname('contador_crc').asstring,                   //Número de inscrição no Conselho Regional:
                          frmmodulo.qrfilial.fieldbyname('contador_cnpj').asstring,                      //CNPJ do escritório de contabilidade, se houver:
                          frmmodulo.qrfilial.fieldbyname('contador_cep').asstring,                       //Código de Endereçamento Postal:
                          frmmodulo.qrfilial.fieldbyname('contador_endereco').asstring,              //Logradouro e endereço do imóvel:
                          frmmodulo.qrfilial.fieldbyname('contador_numero').asstring,                     //Número do imóvel:
                          frmmodulo.qrfilial.fieldbyname('contador_complemento').asstring,             //Dados complementares do endereço:
                          frmmodulo.qrfilial.fieldbyname('contador_bairro').asstring,                  //Bairro em que o imóvel está situado:
                          frmmodulo.qrfilial.fieldbyname('contador_fone').asstring,              //Número do telefone:
                          frmmodulo.qrfilial.fieldbyname('contador_fax').asstring,              //Número do fax:
                          frmmodulo.qrfilial.fieldbyname('contador_email').asstring,        //Endereço do correio eletrônico:
                          frmmodulo.qrfilial.fieldbyname('CONTADOR_COD_MUNICIPIO_IBGE').asstring                   //Código do município, conforme tabela IBGE:
                          );
...
procedure TfrmSpedFiscal.Bloco_0_FechamentoClick(
  Sender: TObject);
var
 TempInt: Integer;
begin
...
procedure TfrmSpedFiscal.Bloco_0_Reg_0190Click(Sender: TObject);
var
 TempInt: Integer;
begin
  ...
  begin
    ...
    begin
      //--- Registro0190 -  ---
       TempInt := Registro0190(query.fieldbyname('unidade').asstring,        // Código da unidade de medida:
                              'UNIDADE'+' em '+query.fieldbyname('unidade').asstring         // Descrição da unidade de medida:
                              );
       Trata_SPED_Int(sender, TempInt);
       query.next;
    end;
...
procedure TfrmSpedFiscal.Bloco_9Click(Sender: TObject);
var
 TempInt: integer;
begin
  (* FINALIZACAO DO SPED*)
...
procedure TfrmSpedFiscal.Bloco_0_Reg_0200Click(Sender: TObject);
var
 TempInt: Integer;
begin
  // tabela de produtos
  ...
  begin
    ...
    begin
      ...
      TempInt := Registro0200(query.fieldbyname('codigo').asstring,                       //Código do item:
                              query.fieldbyname('produto').asstring,                       //Descrição do item:
                              query.fieldbyname('codbarra').asstring,                   //Código de barra do produto, se houver:
                              query.fieldbyname('codigo').asstring,                     //Código anterior do item (ultima apresentado):
                              ...
                              '',                        //Código serviço Anexo I - Lei nº116/03:
                              query.fieldbyname('aliquota').asstring   //Alíquota ICMS aplicável (operações internas):
                              );
      Trata_SPED_Int(sender, TempInt);
      query.next;
    end;
...
procedure TfrmSpedFiscal.Bloco_DClick(Sender: TObject);
var
 TempInt: Integer;
begin
...
procedure TfrmSpedFiscal.Bloco_1Click(Sender: TObject);
var
 TempInt: Integer;
begin
...
procedure TfrmSpedFiscal.Bloco_HClick(Sender: TObject);
var
 TempInt: Integer;
 sQtde, stotal : string;
begin
  ...
  TempInt := RegistroH005(ed_data_fim.Text,           // Data do inventário
  ...
  begin
    ...
    begin
       ...
       TempInt := RegistroH010(
                               query.fieldbyname('codigo').asstring,                     // Código do item (campo 02 do Registro 0200)
                               query.fieldbyname('unidade').asstring,                      // Unidade do item
                               sqtde,                      // Quantidade do item
                               query.fieldbyname('precocusto').asstring,                      // Valor unitário do item
                               ...
                               '1'                    // Código da conta analítica contábil debitada/creditada
        );
...
procedure TfrmSpedFiscal.Bloco_C_AberturaClick(Sender: TObject);
var
 TempInt: Integer;
begin
...
procedure TfrmSpedFiscal.Bloco_C_FechamentoClick(
  Sender: TObject);
var
 TempInt: integer;
BEGIN
...
procedure TfrmSpedFiscal.Bloco_C_Reg_C300Click(Sender: TObject);
var
 TempInt: integer;
  ...
  BEGIN
    ...
    begin
       ...
       TempInt := RegistroC350(
                               qrySintegra.fieldbyname('serie').asstring,                       //  Série do documento fiscal
                               qrysintegra.fieldbyname('subserie').asstring,                       //  Subsérie do documento fiscal
                               qrySintegra.fieldbyname('notafiscal').asstring,                       //  Número do documento fiscal
                               qrySintegra.fieldbyname('data').asstring,                 //  Data da emissão do documento fiscal
       ...
       begin
            ...
            // N I V E L 3
             TempInt := RegistroC370(
                                     query.fieldbyname('num_item').asstring,                       //  Número seqüencial do item no documento fiscal
                                     query.fieldbyname('cd_produto').asstring,                       //  Código do Item (campo 02 do registro 0200)
       ...
       begin
          ...
          //- REGISTRO ANALÍTICO DAS NOTAS FISCAIS DE VENDA A CONSUMIDOR (CÓDIGO 02)
           ...
           TempInt := RegistroC390(
                                   query.fieldbyname('s_trib').asstring,                       //  Código da Situação Tributária, conforme a Tabela indicada no item 4.3.1
                                   query.fieldbyname('cfop').asstring,                       //  Código Fiscal de Operação e Prestação
                                   query.fieldbyname('aliquota_icms').asstring,                       //  Alíquota do ICMS
                                   query.fieldbyname('total_geral').asstring,                       //  Valor total acumulado das operações correspondentes à combinação de CST_ICMS, CFOP e alíquota do ICMS, incluídas as despesas acessórias e acréscimos.
                                   query.fieldbyname('base_icms').asstring,                       //  Valor acumulado da base de cálculo do ICMS, referente à combinação de CST_ICMS, CFOP, e alíquota do ICMS.
                                   query.fieldbyname('valor_icms').asstring,                       //  Valor acumulado do ICMS, referente à combinação de CST_ICMS, CFOP e alíquota do ICMS.
                                   '0',                       //  Valor não tributado em função da redução da base de cálculo do ICMS, referente à combinação de CST_ICMS, CFOP, e alíquota do ICMS.
...
procedure TfrmSpedFiscal.Bloco_C_Reg_C400Click(Sender: TObject);
VAR
 TempInt: Integer;
begin
    ...
    begin
      ...
      //EQUIPAMENTO ECF (CÓDIGO 02 e 2D)
       ...
       TempInt := RegistroC400(
                               ...
                               qrECF.fieldbyname('ecf_serial').asstring,                       //  Número de série de fabricação do ECF
                               qrECF.fieldbyname('ecf_caixa').asstring                        //  Número do caixa atribuído ao ECF
                               );
           ...
           begin
             ...
             TempInt := RegistroC405(
                                     qrreducao.fieldbyname('data_emissao').asstring,               //  Data do movimento a que se refere a Redução Z
                                     qrreducao.fieldbyname('contador_reinicio').asstring,                       //  Posição do Contador de Reinício de Operação
                                     qrreducao.fieldbyname('nro_contador_z').asstring,                       //  Posição do Contador de Redução Z
                                     qrreducao.fieldbyname('nro_contador_fim').asstring,                       //  Número do Contador de Ordem de Operação do último documento emitido no dia. (Número do COO na Redução Z)
             ...
             // N I V E L 4
             //REGISTRO DOS TOTALIZADORES PARCIAIS DA REDUÇÃO Z (COD 02 e 2D) - TOTAL POR ALIQUOTA
             IF qrreducao.fieldbyname('base01').asfloat > 0 then
             begin
               TempInt := RegistroC420(
                                       '01T'+somenteNumero(formatfloat('00.00',qrreducao.fieldbyname('aliquota01').asfloat)), //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('base01').asstring,   //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
                                       '01',                       //  Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva.
             ...
             begin
               TempInt := RegistroC420(
                                       '02T'+somenteNumero(formatfloat('00.00',qrreducao.fieldbyname('aliquota02').asfloat)), //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('BASE02').asstring,   //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
                                       '02',                       //  Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva.
                                       ''                        //  Descrição da situação tributária relativa ao totalizador parcial, quando houver mais de um com a mesma carga tributária efetiva.
                                       );
               Trata_SPED_Int(sender, TempInt);
             end;
             ...
             begin
               TempInt := RegistroC420(
                                       '03T'+somenteNumero(formatfloat('00.00',qrreducao.fieldbyname('aliquota03').asfloat)), //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('BASE03').asstring,   //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
             ...
             begin
               TempInt := RegistroC420(
                                       '04T'+somenteNumero(formatfloat('00.00',qrreducao.fieldbyname('aliquota04').asfloat)), //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('BASE04').asstring,   //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
                                       '04',                       //  Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva.
                                       ''                        //  Descrição da situação tributária relativa ao totalizador parcial, quando houver mais de um com a mesma carga tributária efetiva.
                                       );
               Trata_SPED_Int(sender, TempInt);
             end;
             ...
             begin
               TempInt := RegistroC420(
                                       '05T'+somenteNumero(formatfloat('00.00',qrreducao.fieldbyname('aliquota05').asfloat)), //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('BASE05').asstring,   //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
                                       '05',                       //  Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva.
                                       ''                        //  Descrição da situação tributária relativa ao totalizador parcial, quando houver mais de um com a mesma carga tributária efetiva.
                                       );
               Trata_SPED_Int(sender, TempInt);
             end;
             ...
             begin
               TempInt := RegistroC420(
                                       'F1', //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('SUBSTITUICAO_TRIBUTARIA').asstring,   //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
             ...
             begin
               TempInt := RegistroC420(
                                       'I1', //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('isento').asstring,   //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
                                       '',                       //  Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva.
                                       ''                        //  Descrição da situação tributária relativa ao totalizador parcial, quando houver mais de um com a mesma carga tributária efetiva.
                                       );
               Trata_SPED_Int(sender, TempInt);
             ...
             begin
               TempInt := RegistroC420(
                                       'N1', //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('nao_incidencia').asstring,   //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
                                       '',                       //  Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva.
                                       ''                        //  Descrição da situação tributária relativa ao totalizador parcial, quando houver mais de um com a mesma carga tributária efetiva.
                                       );
               Trata_SPED_Int(sender, TempInt);
             ...
             begin
               TempInt := RegistroC420(
                                       'Can-T',                       //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('cancelamento').asstring,                       //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
                                       '',                       //  Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva.
                                       ''                        //  Descrição da situação tributária relativa ao totalizador parcial, quando houver mais de um com a mesma carga tributária efetiva.
                                       );
               Trata_SPED_Int(sender, TempInt);
             ...
             begin
               TempInt := RegistroC420(
                                       'DT',                       //  Código do totalizador, conforme Tabela 4.4.6
                                       qrreducao.fieldbyname('desconto').asstring,                       //  Valor acumulado no totalizador, relativo à respectiva Redução Z.
                                       '',                       //  Número do totalizador quando ocorrer mais de uma situação com a mesma carga tributária efetiva.
                                       ''                        //  Descrição da situação tributária relativa ao totalizador parcial, quando houver mais de um com a mesma carga tributária efetiva.
                                       );
               Trata_SPED_Int(sender, TempInt);
             ...
             begin
                ...
                begin
                  ...
                  //DOCUMENTO FISCAL EMITIDO POR ECF (CÓDIGO 02 e 2D) - CUPOM FISCAL
                   TempInt := RegistroC460(
                                           '2D',                     //  Código   do   modelo   do   documento   fiscal, conforme a Tabela 4.1.1    --> 00 - OK 02 - CANCELADO
                                           '00',                       //  Código   da   situação   do   documento   fiscal, conforme a Tabela 4.1.2
                                           qrcupom.fieldbyname('numero_cupom_fiscal').asstring,                       //  Número do documento fiscal (COO)
                                           qrcupom.fieldbyname('data').asstring,               //  Data da emissão do documento fiscal
                     ...
                     begin
                      ...
                      //ITENS DO DOCUMENTO FISCAL EMITIDO POR ECF (CÓDIGO 02 e 2D)
                       ...
                       TempInt := RegistroC470(
                                               qrcupom_item.fieldbyname('codproduto').asstring,                       //  Código do item (campo 02 do Registro 0200)
                                               ...
                                               qrcupom_item.fieldbyname('cst').asstring,                       //  Código da Situação Tributária, conforme a Tabela indicada no item 4.3.1.
                                               '5102',                     //  Código Fiscal de Operação e Prestação
                                               qrcupom_item.fieldbyname('aliquota').asstring,                       //  Alíquota do ICMS - Carga tributária efetiva em percentual
                                               '0',                       //  Valor do PIS
                                               '0'                        //  Valor da COFINS
                      );
             ...
             begin
                //REGISTRO ANALÍTICO DO MOVIMENTO DIÁRIO (CÓDIGO 02 E 2D)
                 ...
                 TempInt := RegistroC490(
                                         qrcupom_item.fieldbyname('cst').asstring,                       //  Código da Situação Tributária, conforme a Tabela indicada no item 4.3.1
                                         '5102',                       //  Código Fiscal de Operação e Prestação
                                         qrcupom_item.fieldbyname('aliquota').asstring,                       //  Alíquota do ICMS
                                         qrcupom_item.fieldbyname('valor_total').asstring,                       //  Valor da operação correspondente à combinação de CST_ICMS, CFOP, e alíquota do ICMS, incluídas as despesas acessórias e acréscimos
                                         qrcupom_item.fieldbyname('base_icms').asstring,                       //  Valor acumulado da base de cálculo do ICMS, referente à combinação de CST_ICMS, CFOP, e alíquota do ICMS.
                                         qrcupom_item.fieldbyname('valor_icms').asstring,                       //  Valor acumulado do ICMS, referente à combinação de CST_ICMS, CFOP e alíquota do ICMS.
                                         ''                        //  Código da observação do lançamento fiscal (campo 02 do Registro 0460)
                );
...
procedure TfrmSpedFiscal.Bloco_EClick(Sender: TObject);
var
 TempInt: Integer;
begin
  ...
  TempInt := RegistroE100(
                        ed_data_ini.text,       // Data inicial a que a apuração se refere:
                        ed_data_fim.text        // Data final a que a apuração se refere:
                        );

 //--- RegistroE110 - APURACAO DO ICMS - OPERACOES PROPRIAS ---
  TempInt := RegistroE110(floattostr(rDebito_Icms),    //Valor total dos débitos por "Saídas e prestações com débito do imposto"
                          '0',    //Valor total dos ajustes a débito decorrentes do documento fiscal.
                          '0',    //Valor total de "Ajustes a débito"
                          '0',    //Valor total de Ajustes “Estornos de créditos”
                          '0',    //Valor total dos créditos por "Entradas e aquisições com crédito do imposto"
                          '0',    //Valor total dos ajustes a crédito decorrentes do documento fiscal.
                          '0',    //Valor total de "Ajustes a crédito"
                          floattostr(rDebito_Icms),    //Valor total de Ajustes “Estornos de Débitos”
                          '0',    //Valor total de "Saldo credor do período anterior"
                          '0',    //Valor total de "Saldo devedor (02+03+04+05-06-07-08-09-10) antes das deduções"
                          '0',    //Valor total de "Deduções"
                          '0',    //Valor total de "ICMS a recolher (11-12)
                          '0',    //Valor total de "Saldo credor a transportar para o período seguinte”
    ...
    begin
       TempInt := RegistroE111(
                               'ES039999',                       //  Código do ajuste da apuração e dedução, conforme a Tabela indicada no item 5.1.1.
                               'DEBITO ESPECIAL DE ICMS PARA ES',                       //  Descrição complementar do ajuste da apuração.
                               floattostr(rDebito_Icms)                        //  Valor do ajuste da apuração
      );
