...
uses
  ...
  DBCtrls, DB, AdvGlowButton, RzEdit, ToolEdit, CurrEdit, RXDBCtrl,
  ComCtrls, ZAbstractRODataset, ZAbstractDataset, ZDataset, Menus;
...
uses modulo_nfe, compra_menu, principal, compra_item;
...
begin
  ...
  frmmodulo.qrpreco.fieldbyname('PIS_ENTRADA').asfloat :=
  frmmodulo.qrpreco.fieldbyname('VALOR_ULTIMA_COMPRA').asfloat*
  frmmodulo.qrpreco.fieldbyname('PIS_ENTRADA_P').asfloat/100;

  frmmodulo.qrpreco.fieldbyname('COFINS_ENTRADA').asfloat :=
  frmmodulo.qrpreco.fieldbyname('VALOR_ULTIMA_COMPRA').asfloat*
  frmmodulo.qrpreco.fieldbyname('COFINS_ENTRADA_P').asfloat/100;
...
begin
  ...
  if (frmmodulo.qrconfig.FieldByName('ramo_atividade').asinteger = 2) and
  (FRMMODULO.QRCONFIG.FieldByName('CADASTRO_PRODUTO').asstring = 'NORMAL') then
  begin
  ...
  if frmmodulo.qrproduto.fieldbyname('codmarca').asstring <> '000001' then
    ...
    begin
      ...
      query.Close;
      query.SQL.Clear;
      query.SQL.Add('select * from c000025 where codmarca = '''+frmmodulo.qrproduto.fieldbyname('codmarca').asstring+'''');
      query.Open;
...
//         showmessage(floattostr(frmmodulo.qrpreco.fieldbyname('VALOR_ULTIMA_COMPRA').asfloat)+' Venda '+floattostr(frmmodulo.qrpreco.fieldbyname('PRECO_VENDA').asfloat));
