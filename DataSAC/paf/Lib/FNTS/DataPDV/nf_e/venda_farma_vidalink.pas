...
uses
  ...
  ZAbstractDataset, ZDataset, ZConnection, Mask, ToolEdit, CurrEdit, RzEdit,
...
uses modulo_nfe, senha, principal, venda_fechamento_vidalink, compra_menu,
...
begin
  if evalornf.Value > 0 then
    ...
    begin
      application.messagebox('Inconsistência nos dados, verifique!!','Atenção!',mb_ok+mb_iconwarning);
        ...
        begin
          ...
          //if copy(entrada,1,6) = '000212' then
            ...
            if etpautorizacao.Text = '0' then
              etipo.Text := 'À Vista ( PBM )'
            else if etpautorizacao.Text = '1' then
              etipo.Text := 'Á Receber ( Vidalink  Aberto)'
            else if etpautorizacao.Text = '2' then
              etipo.Text := 'Á Receber ( Convênio Fechado )'
            else if etpautorizacao.Text = '3' then
              etipo.Text := 'Á Receber ( Convênio Aberto  )'
            else if etpautorizacao.Text = '4' then
              etipo.Text := 'Á Receber ( Vidalink  Fechado)';
              ...
              begin
                ShowMessage('Pré-Autorização de Consulta Inválida : ' + emensagem.Text);
            ...
            if eautorizacao.Text <> btautorizacao.Text then
              begin
                if application.messagebox('Autorização diferente da pré-autorização, continua?','Aviso',mb_yesno+MB_ICONWARNING) = idyes then
                else
            ...
            // valor da venda da farmacia(preço cliente-a vista+
...
begin
  if eapresentacao.Value = 0 then
    begin
      application.messagebox('Apresentação não informada, verifique!!','Atenção!',mb_ok+mb_iconwarning);
...
begin
  ...
  if trim(ecpf.Text) = '' then
    begin
      application.messagebox('CPF Inválido, verifique!!','Atenção!',mb_ok+mb_iconwarning);
      ecpf.SetFocus;
    end;
end;
...
begin
  ...
  if esolicitacao.Text = '' then
  begin
    application.messagebox('Não Foi Informado o Código de Solicitação de Medicamentos, verifique!!','Atenção!',mb_ok+mb_iconwarning);
    bconsulta_datasus.setfocus;
    exit;
  end;
  ...
  // Registro Tipo 01 - Login da Farmácia
  ...
  // Registro Tipo 03 - Dados do Estabelecimento e Paciente - Pré-Autorização
  solicitacao := frmPrincipal.zerarcodigo(esolicitacao.Text,6); // Codigo da Solicitação Gerada pelo Estabelecimento (6)
 ...
 // uf := 'RS';
  ...
  emissao := dfpprescricao.Text; // Data de Emissão da Prescricao (10) (dd/mm/aaaa)
  ...
  // Registro Tipo 04 - Dados dos Medicamentos - Pré-Autorização
  ean := ''; // Cód. de barras (EAN) da apresentação do medicamento a ser dispensado (13)
  qtde_solicitada := ''; // Quantidade Solicitada (em comprimidos) (05) (3 inteiros e 2 dec - zeros a esq.)
  preco := ''; // Preco de Venda da Apresentação (15) (13 inteiros e 2 dec - zeros a esq.)
...
begin
  if eautorizacao_datasus.Text <> '' then
    ...
    begin
      application.messagebox('Pré-Autorização Inválida, verifique!!','Atenção!',mb_ok+mb_iconwarning);
      bconsulta_datasus.setfocus;
    end;
end;
  ...
  if qrautorizacao.Locate('autorizacao',qrautorizaAUTORIZACAO.AsString,[loCaseInsensitive]) then
    begin
      if copy(qrautorizacao.FieldByName('obs').AsString,1,7) = 'ESTORNO' then
        begin
          application.messagebox('Autorização já Estornada , Verifique!!','Atenção!',mb_ok+mb_iconwarning);
          bestorno.SetFocus;
          exit;
        end;
    end;

  if application.messagebox(PCHAR('Deseja estornar a Autorizacão de Nº: ' +qrautorizaAUTORIZACAO.AsString+' ?'),'Aviso',mb_yesno+MB_ICONQUESTION) = idyes then
    begin
      ...
      if qrestorno.RecordCount = 0 then
        begin
          application.messagebox('Dados Não Encontrados na Tabela de Movimentação, Verifique!!','Atenção!',mb_ok+mb_iconwarning);
          bestorno.SetFocus;
          exit;
        end;
...
//        showmessage(floattostr(qrestorno.fieldbyname('qtde').AsFloat)+ ' '+floattostr(qrestorno.fieldbyname('apresentacao').AsFloat));
      ...
      // Registro Tipo 03 - Dados do Estabelecimento e Paciente - Pré-Autorização
      autorizacao := qrautorizaAUTORIZACAO.AsString; // Código da Autorização gerado pelo DATASUS (19)
        ...
        begin
          ...
          for x:= 0 to memo1.lines.count-1 do
            ...
            if copy(eretorno_datasus.Text,1,3) = '02E' then
              begin
                Application.MessageBox(pchar(trim(Memo2.Text)),'Atenção',mb_ok+MB_ICONWARNING);
                eretorno_datasus.Text := '';
              end
            else
              begin
...
begin
  if rxprodutos.FieldByName('posologia').AsFloat <= 0 then
    begin
      Application.MessageBox('Quantidade Diária não informada, Verifique!','Atenção',mb_ok+MB_ICONWARNING);
      eposologia.SetFocus;
    end;
end;
...
begin
  if rxprodutos.FieldByName('qtde').AsFloat <= 0 then
    begin
      Application.MessageBox('Quantidade Solicitada não informada, Verifique!','Atenção',mb_ok+MB_ICONWARNING);
      eqtde.SetFocus;
      exit;
    end;
  bgrava_item.SetFocus;
...
begin
  if trim(ecrm.Text) = '' then
    begin
      application.messagebox('CRM Inválido, verifique!!','Atenção!',mb_ok+mb_iconwarning);
      ecrm.SetFocus;
    end;
...
begin
  if trim(euf.Text) = '' then
    begin
      application.messagebox('UF Inválido, verifique!!','Atenção!',mb_ok+mb_iconwarning);
      euf.SetFocus;
    end;
end;
...
begin
  ...
  ecf_relatorio_gerencial(ecf_modelo,'         E S T O R N O   C O N V Ê N I O        ');
  ...
  ecf_relatorio_gerencial(ecf_modelo,'AUTORIZAÇÃO DATASUS : '+ qrautorizaautorizacao.Asstring);
  ecf_relatorio_gerencial(ecf_modelo,'------------------------------------------------');
