...
procedure Tfrmxloc_marca.FormClose(Sender: TObject;
...
procedure Tfrmxloc_marca.locKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = vk_down then
  begin
...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000019 where codigo Is NOT NULL ORDER BY NOME');
  query.open;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000019 where upper('+tipo+') like '''+ref+loc.Text+'%'' order by '+tipo);
    query.open;
  except
  end;
end;
