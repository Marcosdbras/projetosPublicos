...
begin
  ...
  if arvore.Selections[0].Text = 'Boleto Bancário' then
  begin
    pgeral.Visible := false;
    pcadastro.visible := false;
    pmovimento.visible := false;
...
begin
  if application.messagebox('Esta operação limpa o Bando de Dados e Transfere dados de outros sistemas! Deseja continuar?','Atenção',mb_yesno+MB_ICONWARNING+MB_DEFBUTTON2) = idyes then
  begin
...
begin
  ...
  begin
     executeFile('\elpro\Complus\ecf\daruma\Exemplo\Delphi\Daruma_Framework_Delphi7.exe', '', '',1)
