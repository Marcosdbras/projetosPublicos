...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000011 where codigo = ''XXXXXX''');
  query.open;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000011 where upper('+tipo+') like '''+ref+loc.Text+'%'' order by '+tipo);
    query.open;
  except
  end;
end;
