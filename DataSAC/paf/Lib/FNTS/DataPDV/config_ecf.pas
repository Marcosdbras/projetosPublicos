...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, ComCtrls, RXClock, Buttons, Mask, rxToolEdit,
  rxCurrEdit, Menus, AdvMenus;
...
type
  TfrmConfig_ECF = class(TForm)
    ...
    procedure bt_rgClick(Sender: TObject);
    procedure ed_rg_descricaoKeyPress(Sender: TObject; var Key: Char);
...
begin
  ...
  if sMsg <> 'ERRO' then
   begin
    try
     i := ExtractStrings([','],[' '],PChar(sMsg),lista);
     if i>0 then
     begin
      for i:=0 to Lista.Count-1 do
      begin
       if lista[i]<>'' then
        ls_total_nf.Items.Add(Lista[i])
      end;
     end;
    finally
      FreeAndNil(Lista);
  ...
  // Listar Relatorios Gerenciais
  ls_rg.Clear;
  sMsg :=cECF_Verifica_RG(iECF_Modelo);
  Lista:=TStringList.Create;
  if sMsg <> 'ERRO' then
   begin
    try
     i := ExtractStrings([','],[' '],PChar(sMsg),lista);   // Função que extrai substrings de uma String ignorando o espaço em branco
     if i>0 then
     begin
      for i:=0 to Lista.Count-1 do
      begin
       if lista[i]<>'' then
        ls_rg.Items.Add(Lista[i]);
      end;
     end;
    finally
     FreeAndNil(Lista);
    ...
    application.messagebox('Erro ao solicitar Relatorios Gerenciais do ECF!','Erro',mb_ok+mb_iconerror);
...
begin
  ...
  begin
    ...
    begin
      ...
      if rb_icms.Checked then
       ls_aliquota.Items.Add('T' + somentenumero(formatfloat('00.00',ed_aliquota.Value)))
      else
       ls_aliquota.Items.Add('S' + somentenumero(formatfloat('00.00',ed_aliquota.Value)));
    end
    else
      Application.MessageBox(pansichar('Comando não executado!'+#13+
...
procedure TfrmConfig_ECF.bt_rgClick(Sender: TObject);
begin
  if ed_rg_descricao.text <> '' then
  begin
    sMsg := cECF_Programa_RG(iECF_Modelo,edit1.text,ed_rg_descricao.Text);

    if sMsg = 'OK' then
    begin
      Application.MessageBox('Relatório Gerencial gravado com sucesso!',
                              'Aviso',mb_ok+MB_ICONINFORMATION);
      ls_rg.Items.add(ed_rg_descricao.text);
      ls_rg.Refresh;
    end
    else
      Application.MessageBox(pansichar('Comando não executado!'+#13+
...
procedure TfrmConfig_ECF.ed_rg_descricaoKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl,0,0);
end;

procedure TfrmConfig_ECF.TabSheet5Enter(Sender: TObject);
begin
 edit1.setfocus;
end;
