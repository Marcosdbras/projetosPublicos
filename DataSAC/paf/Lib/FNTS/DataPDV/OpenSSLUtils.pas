(******************************************************************************
 Utility module for OpenSSL for Delphi, version 0.3c, 2008-10-23
 For OpenSSL version 0.9.6b, DLL compiled by GnuWin32.
 Tested only on Borland Delphi 6 Personal
...
TMessageSigner = class
  private
    ...
    fPassword: string;
    fSignedMessage: string;
  public
    constructor Create;
    procedure MIMESign;
    procedure LoadPrivateKey(KeyFilename: TFileName); overload;
    procedure LoadPrivateKey(KeyFilename: TFileName; KeyPassword: string); overload;
    ...
    property PlainMessage: string read fMessage write fMessage;
    property SignedMessage: string read fSignedMessage;
end;
...
procedure TMessageSigner.LoadPrivateKey(KeyFilename: TFileName; KeyPassword: string);
var
  keyfile: pBIO;
  pw: PChar;
...
BIO_read_filename(keyfile, PChar(KeyFilename));
if KeyPassword = '' then
  pw := nil
else
  pw := PChar(KeyPassword);
...
certfile := BIO_new(BIO_s_file());
BIO_ctrl(certfile, BIO_C_SET_FILENAME, BIO_CLOSE or BIO_FP_READ, PChar(CertificateFilename));
fCertificate := PEM_read_bio_X509_AUX(certfile, c, nil, nil);
if fCertificate = nil then
