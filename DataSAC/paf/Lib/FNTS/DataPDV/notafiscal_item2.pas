...
begin
  ...
  begin
    frmnotafiscalnova.qrnotafiscal_item.close;
    frmnotafiscalnova.qrnotafiscal_item.sql.clear;
    frmnotafiscalnova.qrnotafiscal_item.SQL.add('insert into c000062');
    frmnotafiscalnova.qrnotafiscal_item.SQL.add('(CODNOTA,CODPRODUTO,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('QTDE,UNITARIO,TOTAL,IPI,ICMS,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('CFOP,CODGRADE,SERIAL,VALOR_IPI,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('CLASSIFICACAO_FISCAL,CST,VALOR_ICMS,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('ICMS_REDUZIDO,BASE_CALCULO,MARGEM_AGREGADA,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('BASE_SUB,ICMS_SUB,ISENTO,CODLANCAMENTO,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('DESCONTO,SUBTOTAL,PESO_BRUTO,PESO_LIQUIDO,Item,codigo,grade,complemento,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('pis_base,pis_valor,pis_aliquota,cofins_base,cofins_valor,cofins_aliquota)');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('VALUES');
    frmnotafiscalnova.qrnotafiscal_item.SQL.add('(:CODNOTA,:CODPRODUTO,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add(':QTDE,:UNITARIO,:TOTAL,:IPI,:ICMS,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add(':CFOP,:CODGRADE,:SERIAL,:VALOR_IPI,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add(':CLASSIFICACAO_FISCAL,:CST,:VALOR_ICMS,');
    ...
    frmnotafiscalnova.qrnotafiscal_item.sql.add(':DESCONTO,:SUBTOTAL,:PESO_BRUTO,:PESO_LIQUIDO,:ITEM,:codigo,:grade,:complemento,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add(':pis_base,:pis_valor,:pis_aliquota,:cofins_base,:cofins_valor,:cofins_aliquota)');
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('CODNOTA'             ).asstring := frmnotafiscal_menu.qrnota.fieldbyname('codigo').asstring;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('CODPRODUTO'          ).asstring := ed_codigo.text;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('QTDE'                ).asfloat  := ed_qtde.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('UNITARIO'            ).asfloat  := ed_unitario.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('TOTAL'               ).asfloat  := ed_total.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('IPI'                 ).asfloat  := ed_ipi_aliquota.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('ICMS'                ).asfloat  := ed_icms_aliquota.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('CFOP'                ).asstring := ed_cfop.Text;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('CODGRADE'            ).asstring := ed_grade_codigo.Text;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('SERIAL'              ).asstring := copy(ed_serial.Text,1,25);
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('VALOR_IPI'           ).asfloat  := ed_ipi_valor.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('CLASSIFICACAO_FISCAL').asstring := ed_classific.text;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('CST'                 ).asstring := ed_cst.Text;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('VALOR_ICMS'          ).asfloat  := ed_icms_valor.Value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('ICMS_REDUZIDO'       ).asfloat  := ed_icms_reducao.Value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('BASE_CALCULO'        ).asfloat  := ed_icms_base.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('MARGEM_AGREGADA'     ).asfloat  := margem_agregada;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('BASE_SUB'            ).asfloat  := ed_sub_base.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('ICMS_SUB'            ).asfloat  := ed_sub_valor.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('ISENTO'              ).asfloat  := ed_isenta.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('CODLANCAMENTO'       ).asstring := frmprincipal.codifica('000032',10);
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('DESCONTO'            ).asfloat  := ed_desconto.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('SUBTOTAL'            ).asfloat  := ed_subtotal.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('PESO_BRUTO'          ).asfloat  := ed_peso_bruto.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('PESO_LIQUIDO'        ).asfloat  := ed_peso_liquido.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('Item'                ).asinteger:= nf_item;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('codigo'              ).asstring := frmprincipal.codifica('000062',6);
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('GRADE'               ).asstring := ED_grade.text;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('COMPLEMENTO'         ).AsBlob := ED_COMPLEMENTO.Lines.Text;


    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('pis_base'            ).asfloat  := ed_Pis_Base.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('pis_valor'           ).asfloat  := ed_Pis_valor.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('pis_aliquota'        ).asfloat  := ed_Pis_aliquota.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('cofins_base'         ).asfloat  := ed_cofins_Base.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('cofins_valor'        ).asfloat  := ed_cofins_valor.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('cofins_aliquota'     ).asfloat  := ed_cofins_aliquota.value;


    frmnotafiscalnova.qrnotafiscal_item.ExecSQL;
  ...
  begin
    frmnotafiscalnova.qrnotafiscal_item.close;
    frmnotafiscalnova.qrnotafiscal_item.sql.clear;
    frmnotafiscalnova.qrnotafiscal_item.SQL.add('update c000062 set');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('QTDE                 = :QTDE,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('UNITARIO             = :UNITARIO,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('TOTAL                = :TOTAL,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('IPI                  = :IPI,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('ICMS                 = :ICMS,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('CFOP                 = :CFOP,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('VALOR_IPI            = :VALOR_IPI,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('CLASSIFICACAO_FISCAL = :CLASSIFICACAO_FISCAL,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('CST                  = :CST,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('VALOR_ICMS           = :VALOR_ICMS,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('ICMS_REDUZIDO        = :ICMS_REDUZIDO,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('BASE_CALCULO         = :BASE_CALCULO,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('MARGEM_AGREGADA      = :MARGEM_AGREGADA,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('BASE_SUB             = :BASE_SUB,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('ICMS_SUB             = :ICMS_SUB,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('ISENTO               = :ISENTO,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('DESCONTO             = :DESCONTO,');
    {PESO_NOVO}
    frmnotafiscalnova.qrnotafiscal_item.sql.add('PESO_BRUTO           = :PESO_BRUTO,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('PESO_LIQUIDO         = :PESO_LIQUIDO,');

    frmnotafiscalnova.qrnotafiscal_item.sql.add('PIS_BASE             = :PIS_BASE,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('PIS_VALOR            = :PIS_VALOR,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('PIS_ALIQUOTA         = :PIS_ALIQUOTA,');

    frmnotafiscalnova.qrnotafiscal_item.sql.add('cofins_BASE             = :cofins_BASE,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('cofins_VALOR            = :cofins_VALOR,');
    frmnotafiscalnova.qrnotafiscal_item.sql.add('cofins_ALIQUOTA         = :cofins_ALIQUOTA,');


    frmnotafiscalnova.qrnotafiscal_item.sql.add('SUBTOTAL             = :SUBTOTAL');

    frmnotafiscalnova.qrnotafiscal_item.sql.add(' WHERE CODIGO = '''+PCODIGO.CAPTION+'''');
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('QTDE'                ).asfloat  := ed_qtde.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('UNITARIO'            ).asfloat  := ed_unitario.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('TOTAL'               ).asfloat  := ed_total.value;
    frmnotafiscalnova.qrnotafiscal_item.Params.Parambyname('IPI'                 ).asfloat  := ed_ipi_aliquota.value;
  ...
  begin
    ...
    if frmnotafiscalnova.check_desconto.Checked = false then
...
begin
     ...
     vdesconto_anterior := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('DESCONTO').asfloat;
     qtde_anterior      := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('qtde').asfloat;
     valor_anterior     := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('subtotal').asfloat;
     icms_anterior      := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('valor_icms').asfloat;
     ipi_anterior       := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('valor_ipi').asfloat;
     base_anterior      := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('base_calculo').asfloat;
     base_sub_anterior  := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('base_sub').asfloat;
     icms_sub_anterior  := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('icms_sub').asfloat;
     PIS_anterior       := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('PIS_VALOR').asfloat;
     COFINS_anterior    := frmnotafiscalnova.qrNotaFiscal_Item.fieldbyname('COFINS_VALOR').asfloat;
