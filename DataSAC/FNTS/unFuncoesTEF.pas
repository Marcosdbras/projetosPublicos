...
var
  ...
  sTEFKoneKPath      : string  = 'C:\DataSAC\TEF_DIAL\';
  sTEFRetorno        : string;
  sTEFResposta       : string;
...
uses principal, modulo, ecf_bematech, ecf_daruma;
...
begin
 ...
 {   iInicio := SecondOf(Now());
    ...
    tempo_gp := 0;
    frmmodulo.time_gp.Enabled := true;
     repeat // LOOPING --> ESPERAR 7 SEGUNDOS OU VERIFICAR SE EXISTE O ARQUIVO INTPOS.STS
      application.ProcessMessages;
    until (FileExists(sNomeArquivo) or (tempo_gp > iTEFTempoEspera));


    frmmodulo.time_gp.enabled := false;
...
//    begin
    if sTEFRetorno <> '0' then
    begin
//      application.messagebox('Não houve resposta do Gerenciador Padrão!','Atenção!',mb_ok+mb_iconerror);
    end;
//    end;
      ...
      begin
        ...
        BEGIN
          if sValor <> '' then
            ...
            sTextoValor := '';
            texto_msg := pansichar('Cancelada a transação:'+#13+#10+('Rede: '+Trim(sRede))+#13+#10+('Doc No: ' + Trim(sDocto))+#13+#10+sTextoValor);
            application.messagebox(texto_msg,'Atenção',mb_ok+mb_iconwarning);
