...
uses
  ...
  frxDBSet, frxDesgn, Menus, Grids, DBGrids, wwdblook;
...
uses modulo, xloc_cliente, principal,
  xloc_banco;
...
begin
  ...
  begin
    QRCHEQUE.close;
    QRCHEQUE.sql.clear;
    QRCHEQUE.sql.add('select ch.*, cli.nome, cli.telefone1, cli.telefone2 from c000040 ch, c000007 cli where ch.codcliente = cli.codigo AND '+periodo+cliente+banco+situacao+' order by cliente');
    QRCHEQUE.Params.ParamByName('datai').asdatetime := dateedit1.date;
    QRCHEQUE.Params.ParamByName('dataf').asdatetime := dateedit2.date;
    QRCHEQUE.open;
    QRCHEQUE.REFRESH;
