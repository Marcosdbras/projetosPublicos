...
uses
  ...
  Dialogs, ExtCtrls, rxtooledit, RXDBCtrl, StdCtrls, Mask, DBCtrls, DB,
  ZAbstractRODataset, ZAbstractDataset, ZDataset, Grids, Wwdbigrd, Wwdbgrid,
  rxcurredit, Buttons, wwdbedit, Menus, TFlatPanelUnit, Spin, wwdblook,
  Wwdbdlg, AdvGlowButton;
...
type
  Tfrmecf_cupom = class(TForm)
    ...
    qritensNUMERONOTA: TStringField;
    qritensCODCLIENTE: TStringField;
    qritensDESCONTO: TFloatField;
    qritensACRESCIMO: TFloatField;
    qritensMOVIMENTO: TIntegerField;
    qritensCODVENDEDOR: TStringField;
    qritensCODGRADE: TStringField;
    qritensSERIAL: TStringField;
    ...
    qritensECF_SERIE: TStringField;
    qritensECF_CAIXA: TStringField;
    qritensCODALIQUOTA: TStringField;
    qritensCUPOM_NUMERO: TStringField;
    qritensCUPOM_MODELO: TStringField;
...
begin
  ...
  begin
    if application.messagebox('Existe intes em edição! Deseja confirmar?','Aviso',mb_yesno+mb_iconquestion) = idyes then
    begin
...
begin
  if application.messagebox('Confirma a exclusão?','Aviso',mb_yesno+MB_ICONWARNING) = idyes then
  begin
    qritens.delete;
  end;
