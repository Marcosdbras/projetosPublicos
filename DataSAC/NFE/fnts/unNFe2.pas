...
begin
 ...
 (* emitente *)
  ...
  (* destinatario *)
    ...
    if destinatario_cnpj <> '' then
      Writeln(txt_nf,'E02|'+SN(Destinatario_CNPJ));
    if destinatario_cpf <> '' then
      Writeln(txt_nf,'E03|'+SN(Destinatario_CPF));
  ...
  begin
    ...
    begin
      writeln(txt_nf,'N10|'+Inttostr(Origem)+'|'+
                            ...
                            FVz(ICMS_Valor,2)+'|'+
                            Inttostr(ST_Modalidade)+'|'+
                            FVz(ST_Margem_Adicional,2)+'|'+
                            FVz(ST_Reducao,2)+'|'+
                            FVz(ST_Base_Calculo,2)+'|'+
                            FVz(ST_Aliquota,2)+'|'+
                            FVz(ST_Valor,2));
    end;
