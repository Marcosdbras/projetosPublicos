unit contasreceber_cedente_alterar;
...
type
  Tfrmcontasreceber_cedente_alterar = class(TForm)
    Label10: TLabel;
    Label11: TLabel;
    Label12: TLabel;
    Label13: TLabel;
...
var
  frmcontasreceber_cedente_alterar: Tfrmcontasreceber_cedente_alterar;
...
procedure Tfrmcontasreceber_cedente_alterar.BCANCELARClick(Sender: TObject);
begin
   close;
end;

procedure Tfrmcontasreceber_cedente_alterar.Fechar1Click(Sender: TObject);
begin
  close;
end;

procedure Tfrmcontasreceber_cedente_alterar.FormShow(Sender: TObject);
begin
...
procedure Tfrmcontasreceber_cedente_alterar.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  action := cafree;
end;

procedure Tfrmcontasreceber_cedente_alterar.COMBOCONTAEnter(Sender: TObject);
begin
  tedit(sender).color := clyellow;
end;

procedure Tfrmcontasreceber_cedente_alterar.COMBOCONTAExit(Sender: TObject);
begin
...
procedure Tfrmcontasreceber_cedente_alterar.COMBOCONTAKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then perform(wm_nextdlgctl,0,0);
end;

procedure Tfrmcontasreceber_cedente_alterar.Edit1Exit(Sender: TObject);
begin
  tedit(sender).color := clwindow;
end;

procedure Tfrmcontasreceber_cedente_alterar.Edit3KeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then bgravar.SetFocus;
end;

procedure Tfrmcontasreceber_cedente_alterar.bcobrebemClick(Sender: TObject);
var
   CobreBemX: Variant;
   Boleto: Variant;
...
begin
     ...
     CobreBemX.PadroesBoleto.PadroesBoletoImpresso.CaminhoImagensCodigoBarras := 'c:\elpro\Complus\cob\CobreBemX\Imagens\';
    ...
    frmmodulo.qrcliente.close;
    frmmodulo.qrcliente.sql.Clear;
    frmmodulo.qrcliente.sql.add('select * from c000007 where codigo = '''+frmcontasreceber.qrcontasreceber.fieldbyname('codcliente').asstring+'''');
    frmmodulo.qrcliente.open;
    ...
    begin
      ...
      begin
        ...
        begin
           ...
           Boleto.CepSacado       := frmmodulo.qrcliente.fieldbyname('cep').asstring;
           ...
           Boleto.ValorDocumento  := frmcontasreceber.qrcontasreceber.fieldbyname('valor_atual').asfloat;
...
procedure Tfrmcontasreceber_cedente_alterar.BGRAVARClick(Sender: TObject);
begin
...
procedure Tfrmcontasreceber_cedente_alterar.BMATRICIALClick(Sender: TObject);
var f : textfile;
...
// CONDENSADO NEGRITO   --> #15+#27#71+#20
  ...
  assignfile(f,'\\elpro-serv\Epsonlx300');
  rewrite(f);
  frmmodulo.qrcliente.close;
  frmmodulo.qrcliente.sql.Clear;
  frmmodulo.qrcliente.sql.add('select * from c000007 where codigo = '''+frmcontasreceber.qrcontasreceber.fieldbyname('codcliente').asstring+'''');
