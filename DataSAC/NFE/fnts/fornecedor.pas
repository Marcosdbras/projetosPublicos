...
begin
  ...
  frmmodulo.qrFORNECEDOR.fieldbyname('codigo').asstring := frmprincipal2.codifica('000009');
...
begin
  if frmprincipal2.autentica('Excluir FORNECEDOR',4) then
  begin
...
begin
  ...
  IF CONTINUA_INCLUSAO THEN
    frmprincipal2.agenda('1','FORNECEDOR',DBEdit2.Text,DBEdit10.Text,DBEdit11.Text,'','',DBEdit13.Text,DBEdit12.Text,DBEdit28.Text,'3');
...
begin
  tipo := 'CNPJ';
  CAMPO := PANSICHAR(FRMPRINCIPAL2.somenteNumero(FRMMODULO.QRFORNECEDOR.FIELDBYNAME('CNPJ').ASSTRING));
end
else
begin
  ...
  begin
    tipo := 'IE';
    CAMPO := PANSICHAR(FRMPRINCIPAL2.somenteNumero(FRMMODULO.QRFORNECEDOR.FIELDBYNAME('IE').ASSTRING));
  end
  else
  begin
...
begin
tedit(sender).Color := clwindow;
frmmodulo.qrfornecedor.fieldbyname('numero').asstring := frmprincipal2.zerarcodigo(dbedit16.text,5);
end;
