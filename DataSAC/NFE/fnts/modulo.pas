...
uses principal, ecf, produto, principal2;
...
//  caminho := '\DataSAC\server\dll\com_7.ini';

  caminho := '\DATASAC\server\cfg\config.ini';         // Abre o Sistema Atual
  BuscaChave := 'Ecfcom.dll';

  //caminho := '\DATASAC\server\dll\comFilial.ini';  // Abre um Segundo Sistema
  //BuscaChave := 'EcfcomFilial.dll';
      
  if caminho <> '\DATASAC\server\cfg\config.ini' then
  begin
    frmprincipal2.pfundo.color := $00C6F3FB;
    frmprincipal2.pfundo.ColorShadow := $00C6F3FB;
    frmprincipal2.pfundo.ColorHighLight := $00C6F3FB;
  end;
  ...
  begin
    ...
    begin
      if trim(copy(entrada,9,1)) = '' then
        frmprincipal2.caption := 'DataSAC 2.0 | Server - PAF 1.09                [ Servidor Local ]'
      else
        frmprincipal2.caption := 'DataSAC 2.3 | Cliente- PAF 1.09                [ Servidor: '+trim(copy(entrada,9,50))+' ]';
    end;
...
begin
  qrlog.FieldValues['codigo'] :=  frmprincipal2.codifica('000002');
end;
...
begin
  ...
  begin
    frmmodulo.qrsubgrupo.fieldbyname('codigo').asstring := frmprincipal2.codifica('000018');
...
begin
  frmmodulo.qrgrade_subgrupo.fieldbyname('codigo').asstring := frmprincipal2.codifica('000020');
...
begin
  qragenda.FieldValues['codigo'] := frmprincipal2.codifica('000034');
end;
