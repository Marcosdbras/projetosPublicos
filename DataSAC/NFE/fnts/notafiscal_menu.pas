...
uses modulo, principal, compra, cfop, cst, modelo_fiscal, xloc_cliente,
  ...
  status, nfe_legenda, email, math, principal2;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Prod.cEAN     := frmPrincipal2.Zerarcodigo(qrnota_item.fieldbyname('codbarra').asstring, 13);
...
begin
  ...
  qrnota.Insert;
  qrnota.FieldByName('codigo').asstring := frmprincipal2.codifica('000061');
...
begin
  ...
  if frmprincipal2.autentica('Excluir NF',4) then
    begin
      query2.close;
      query2.sql.clear;
...
begin
  ...
  begin
    ...
    qrcliente2.sql.add('select * from c000007 where codigo = '''+frmprincipal2.zerarcodigo(copy(ecliente.Text,1,6),6)+'''');
    qrcliente2.Open;
...
begin
  enumero.text := frmprincipal2.zerarcodigo(enumero.text,6);
...
begin
 ...
 if frmprincipal2.autentica('Cancelar NF',4) then
  begin
    if QRNOTA.fieldbyname('situacao').AsInteger = 1 then
    begin
...
begin
  ...
  if frmprincipal2.autentica('Zerar NF',4) then
  begin
...
begin
  ...
  begin
    ...
    if not Exp_Inicia_NotaFiscal(1,
                            ...
                            '55', // modelo
                            '001', // serie
                            frmprincipal2.zerarcodigo(qrnota.fieldbyname('numero').asstring,8), // número da nf
    ...
    begin
          ...
          if NOT Exp_Inclui_item(
                                ...
                                frmprincipal2.zerarcodigo(qrnota_item.fieldbyname('cod_barra').asstring,13),//Cod_Barra,
    ...
    begin
      ...
      if not Exp_Faturamento_NotaFiscal(
                                        frmprincipal2.zerarcodigo(qrnota.fieldbyname('numero').asstring,9),//Numero_Fatura:string;
...
begin
  try
    sXML :=qrnota.FieldByName('nfe_xml').asstring ;  //Gerar_XML;
    ...
    begin
      ...
      FrmSendEmail.EdtPara.Text:= para;
      FrmSendEmail.LbAnexos.Items.Add(sXML);
      frmsendemail.Edit1.Text:=trim(sxml);
