...
begin
 ...
 df_entrada.date := frmprincipal2.UltimoDiaMes(date);

 di_saida.Text := '01/'+copy(datetostr(date),4,7);
 df_saida.date := frmprincipal2.UltimoDiaMes(date);
...
begin
  ...
  qrproduto.fieldbyname('data_inventario').AsDateTime := strtodate('31/12/'+frmprincipal2.zerarcodigo(inttostr(strtoint(copy(datetostr(date),7,4))-1),4));
  qrproduto.fieldbyname('custo_inventario').AsFloat := 0;
  qrproduto.fieldbyname('estoque_inventario').AsFloat := 0;

  qrproduto.fieldbyname('codigo').asstring := frmprincipal2.codifica('000025');
...
begin
  if frmprincipal2.autentica('Excluir PRODUTO',4) then
  begin
      qrcodbarra.close;
      qrcodbarra.SQL.clear;
      ...
      END;
       ...
       begin
            frmprincipal2.log(codigo_usuario,'PRODUTO',qrproduto.fieldbyname('codigo').asstring,'EXCLUIU',qrPRODUTO.fieldbyname('PRODUTO').asstring);
        qrproduto.Delete;
...
begin
  ...
  pficha9.enabled := false;
    ...
    if frmmodulo.qrproduto.State = dsinsert then
       frmprincipal2.log(codigo_usuario,'PRODUTO',qrproduto.fieldbyname('codigo').asstring,'INCLUIU',qrPRODUTO.fieldbyname('PRODUTO').asstring);
    ...
    begin
      ...
      if (qrproduto.State = dsinsert) then
        begin
          ...
          frmprincipal2.qrmestre.sql.add('update c000000 set sequencia = sequencia - 1 where codigo = ''000025''');
...
begin
  ...
  begin
    ...
    begin
      if not frmprincipal2.Locregistro(frmmodulo.qrALIQUOTA,eALIQUOTA.text,'codigo') then blocALIQUOTAclick(frmproduto) else begin PageView1.ActivePageIndex := 0; dbmemo1.SetFocus; end;
    end
    else
...
begin
  ...
  qrpreco.insert;
  qrpreco.fieldbyname('codigo').asstring := frmprincipal2.codifica('000026');
...
begin
  ...
  BEGIN
     ...
     begin
       ...
       frmmodulo.qrgrade_produto.fieldbyname('codigo').asstring := frmprincipal2.codifica('000021');
...
begin
  if frmprincipal2.autentica('Excluir GRADE',4) then
  begin
...
begin
  ...
  if frmprincipal2.autentica('Baixar Serial',4) then
  begin
    ...
    begin
      ...
      begin
       ...
       (****************************************)
                ...
                qrproduto_mov.fieldbyname('codigo').AsString := frmprincipal2.codifica('000032');
...
begin
  ...
  IF (qrproduto.state = dsinsert) or (qrproduto.State = dsedit) then
  begin
    qrproduto.fieldbyname('codreceita').asstring := frmprincipal2.zerarcodigo(ereceita.text,6);
    if ereceita.text <> '000000' then
      if not frmprincipal2.Locregistro(frmmodulo.qrreceita,ereceita.text,'codigo') then blocreceitaclick(frmproduto) else wwdbgrid3.setfocus
    else
...
begin
  qrcomposicao_produto.fieldbyname('codigo').asstring := frmprincipal2.codifica('000027');
...
begin
  if frmprincipal2.autentica('Excluir COMPOSIÇÃO',4) then
  begin
...
begin
  ...
  frmmodulo.qrinfnutricional_produto.fieldbyname('codigo').asstring := frmprincipal2.codifica('000028');
...
begin
  if frmprincipal2.autentica('Atualizar Produtos',4) then
  begin
...
begin
  ...
  if Length(Trim(frmprincipal2.somenteNumero(COMBOCODBARRA.Text))) >= 13 then
    begin

      try
        cd := copy(combocodbarra.Text,1,12);
        x := copy(frmprincipal2.somenteNumero(combocodbarra.Text),13,1);
        if x <> '' then
          begin
            if frmprincipal2.CalculaDigEAN13(frmprincipal2.somenteNumero(combocodbarra.Text)) = x then pbarra.visible := false
            else
              begin
...
begin
 ...
 begin
  ...
  QRCODBARRA.FieldByName('CODIGO').AsString := frmprincipal2.codifica('000055');
...
begin
  ...
  begin
    IF (qrproduto.state = dsinsert) or (qrproduto.State = dsedit) then
    begin
      qrproduto.fieldbyname('codGRUPO').asstring := frmprincipal2.zerarcodigo(egrupo.text,6);
      if egrupo.text <> '000000' then
        if not frmprincipal2.Locregistro(frmmodulo.qrGRUPO,egrupo.text,'codigo') then
        begin
...
begin
  ...
  IF (qrproduto.state = dsinsert) or (qrproduto.State = dsedit) then
  begin
    qrproduto.fieldbyname('codsubgrupo').asstring := frmprincipal2.zerarcodigo(esubgrupo.text,6);
    if esubgrupo.text <> '000000' then
      if not frmprincipal2.Locregistro(frmmodulo.qrsubgrupo,esubgrupo.text,'codigo') then
      begin
...
begin
  ...
  IF (qrproduto.state = dsinsert) or (qrproduto.State = dsedit) then
  begin
    qrproduto.fieldbyname('codfornecedor').asstring := frmprincipal2.zerarcodigo(efornecedor.text,6);
...
begin
  ...
  IF (qrproduto.state = dsinsert) or (qrproduto.State = dsedit) then
  begin
    qrproduto.fieldbyname('codmarca').asstring := frmprincipal2.zerarcodigo(emarca.text,6);
    if emarca.text <> '000000' then
      if not frmprincipal2.Locregistro(frmmodulo.qrmarca,emarca.text,'codigo') then
      begin
...
begin
  if frmprincipal2.autentica('Excluir RENTABILIDADE',4) then
  begin
...
begin
 ...
 begin
    ...
    qrrentabilidade.fieldbyname('codigo').asstring := frmprincipal2.codifica('000078');
...
begin
  if key = #13 then
  begin
    elocproduto.text := frmprincipal2.zerarcodigo(elocproduto.text,6);
...
var recor : string;
begin
  if frmprincipal2.autentica('Movimentar Estoque',4) then
  begin
...
begin
  ...
  qrproduto.fieldbyname('codigo').asstring :=  frmprincipal2.zerarcodigo(qrproduto.fieldbyname('codigo').asstring,6);
  ...
  //complus
    ...
    BEGIN
         ...
         frmmodulo.qradic.SQL.add('values');
         frmmodulo.qradic.SQL.add('('''+frmprincipal2.codifica('000032')+''',''000000'',''000000'',''000000'',');
...
procedure Tfrmproduto.AtualizarPreosdeVendasRemarcados1Click(
  Sender: TObject);
begin
  if frmprincipal2.autentica('Atualiza Preços Remarcados',4) then
    begin
    ...
    begin
      ...
      while not query.Eof do begin
        if QUERY1.LOCATE('codproduto',query.fieldbyname('codigo').AsString,[loCaseInsensitive]) THEN
          ...
          begin
            query1.Insert;
            query1.fieldbyname('codigo').asstring               := frmprincipal2.codifica('000026');
...
begin
  ...
  begin
    IF (qrproduto.state = dsinsert) or (qrproduto.State = dsedit) then
    begin
      qrproduto.fieldbyname('CLASSIFICACAO_FISCAL').asstring := frmprincipal2.zerarcodigo(eNCM.text,6);
      if eNCM.text <> '000000' then
        if not frmprincipal2.Locregistro(frmmodulo.qrNCM,eNCM.text,'NCM') then
        begin
...
begin
  ...
  IF (qrproduto.state = dsinsert) or (qrproduto.State = dsedit) then
  begin
    qrproduto.fieldbyname('CSOSN').asstring := frmprincipal2.zerarcodigo(ecsosn.text,3);
    if ecsosn.text <> '000' then
      if not frmprincipal2.Locregistro(frmmodulo.qrCSOSN,ecsosn.text,'codigo') then
      begin
