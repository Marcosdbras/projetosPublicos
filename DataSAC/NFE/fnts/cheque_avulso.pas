...
uses modulo, principal, loc_regiao, regiao, loc_funci, loc_cliente,
  ...
  xloc_cheque, cheque_menu, principal2;
...
begin
  ...
  frmmodulo.qrcheque.fieldbyname('codigo').asstring := frmprincipal2.codifica('000040');
...
begin
  ...
  IF (frmmodulo.qrcheque.state = dsinsert) or (frmmodulo.qrcheque.State = dsedit) then
  begin
    frmmodulo.qrcheque.fieldbyname('codbanco').asstring := frmprincipal2.zerarcodigo(ebanco.text,3);
    if ebanco.text <> '000' then
      if not FrmPrincipal2.Locregistro(frmmodulo.qrbanco,ebanco.text,'numero') then blocbancoclick(frmcheque_avulso) else dbedit4.setfocus
    else
      blocbanco.SetFocus;
  end;
end;
...
begin
  ...
  IF (frmmodulo.qrcheque.state = dsinsert) or (frmmodulo.qrcheque.State = dsedit) then
  begin
    frmmodulo.qrcheque.fieldbyname('codcliente').asstring := frmprincipal2.zerarcodigo(ecliente.text,6);
    if ecliente.text <> '000000' then
    BEGIN
      if not FrmPrincipal2.Locregistro(frmmodulo.qrcliente,ecliente.text,'codigo') then
      BEGIN
