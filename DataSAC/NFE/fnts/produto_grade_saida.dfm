object frmproduto_grade_saida: Tfrmproduto_grade_saida
  Left = 292
  Top = 187
  Width = 263
  Height = 323
  BorderIcons = []
  Caption = 'SAIDA - Grade'
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Verdana'
  Font.Style = []
  OldCreateOrder = False
  PopupMenu = Pop2
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object ViewSplit1: TViewSplit
    Left = 0
    Top = 247
    Width = 255
    Height = 3
    Cursor = crVSplit
    Align = alBottom
  end
  object wwDBGrid1: TwwDBGrid
    Left = 0
    Top = 0
    Width = 255
    Height = 195
    Selected.Strings = (
      'NUMERACAO'#9'20'#9'N'#218'MERO'
      'QTDE'#9'10'#9'QTDE.')
    IniAttributes.Delimiter = ';;'
    TitleColor = clBtnFace
    FixedCols = 1
    ShowHorzScrollBar = True
    Align = alClient
    DataSource = dsgrade_produto
    KeyOptions = [dgEnterToTab, dgAllowDelete, dgAllowInsert]
    TabOrder = 0
    TitleAlignment = taLeftJustify
    TitleFont.Charset = ANSI_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Verdana'
    TitleFont.Style = []
    TitleLines = 1
    TitleButtons = False
    OnEnter = wwDBGrid1Enter
    OnExit = wwDBGrid1Exit
  end
  object pgravar: TFlatPanel
    Left = 0
    Top = 250
    Width = 255
    Height = 39
    Color = clWindow
    ColorHighLight = clWindow
    ColorShadow = clWindow
    Align = alBottom
    TabOrder = 1
    UseDockManager = True
    object bgravar: TBitBtn
      Left = 64
      Top = 6
      Width = 129
      Height = 25
      Caption = 'Fechar'
      TabOrder = 0
      OnClick = bgravarClick
      Glyph.Data = {
        }
    end
  end
  object FlipPanel1: TFlipPanel
    Left = 0
    Top = 195
    Width = 255
    Height = 52
    Align = alBottom
    BorderStyle = bsSingle
    ParentColor = False
    object Label1: TLabel
      Left = 4
      Top = 0
      Width = 116
      Height = 12
      Caption = 'QTDE. DO PRODUTO:'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -9
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 4
      Top = 15
      Width = 98
      Height = 12
      Caption = 'QTDE. DIGITADA:'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -9
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 4
      Top = 31
      Width = 68
      Height = 12
      Caption = 'DIFEREN'#199'A:'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -9
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
    end
    object RTOTAL: TRxCalcEdit
      Left = 128
      Top = 1
      Width = 49
      Height = 17
      AutoSize = False
      BorderStyle = bsNone
      DisplayFormat = '###,###,##0.00'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -9
      Font.Name = 'Verdana'
      Font.Style = []
      ButtonWidth = 0
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
    end
    object RDIGITADO: TRxCalcEdit
      Left = 120
      Top = 15
      Width = 57
      Height = 17
      AutoSize = False
      BorderStyle = bsNone
      DisplayFormat = '###,###,##0.00'
      Font.Charset = ANSI_CHARSET
      Font.Color = clBlue
      Font.Height = -9
      Font.Name = 'Verdana'
      Font.Style = []
      ButtonWidth = 0
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 1
    end
    object RDIFERENCA: TRxCalcEdit
      Left = 120
      Top = 31
      Width = 57
      Height = 17
      AutoSize = False
      BorderStyle = bsNone
      DisplayFormat = '###,###,##0.00'
      Font.Charset = ANSI_CHARSET
      Font.Color = clRed
      Font.Height = -9
      Font.Name = 'Verdana'
      Font.Style = []
      ButtonWidth = 0
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 2
      OnChange = RDIFERENCAChange
    end
  end
  object dsgrade_produto: TDataSource
    DataSet = qrgrade_saida
    Left = 33
    Top = 25
  end
  object qrgrade_saida: TZQuery
    Connection = frmmodulo.Conexao
    BeforeInsert = qrgrade_saidaBeforeInsert
    AfterInsert = qrgrade_saidaAfterInsert
    BeforeEdit = qrgrade_saidaBeforeEdit
    AfterPost = qrgrade_saidaAfterPost
    Active = True
    SQL.Strings = (
      'select * from c000033')
    Params = <>
    Left = 168
    Top = 120
    object qrgrade_saidaNUMERACAO: TStringField
      DisplayLabel = 'N'#218'MERO'
      DisplayWidth = 20
      FieldKind = fkLookup
      FieldName = 'NUMERACAO'
      LookupDataSet = frmmodulo.qrgrade_produto
      LookupKeyFields = 'CODIGO'
      LookupResultField = 'NUMERACAO'
      KeyFields = 'CODGRADE'
      Lookup = True
    end
    object qrgrade_saidaQTDE: TFloatField
      DisplayLabel = 'QTDE.'
      DisplayWidth = 10
      FieldName = 'QTDE'
    end
    object qrgrade_saidaCODGRADE: TStringField
      DisplayWidth = 6
      FieldName = 'CODGRADE'
      Visible = False
      Size = 6
    end
    object qrgrade_saidaCODIGO: TStringField
      DisplayWidth = 6
      FieldName = 'CODIGO'
      Required = True
      Visible = False
      Size = 6
    end
    object qrgrade_saidaCODPRODUTO: TStringField
      DisplayWidth = 6
      FieldName = 'CODPRODUTO'
      Visible = False
      Size = 6
    end
    object qrgrade_saidaCODNOTA: TStringField
      DisplayWidth = 6
      FieldName = 'CODNOTA'
      Visible = False
      Size = 6
    end
    object qrgrade_saidaCODITEM: TStringField
      DisplayWidth = 6
      FieldName = 'CODITEM'
      Visible = False
      Size = 6
    end
  end
  object Pop2: TPopupMenu
    Left = 136
    Top = 120
    object Gravar1: TMenuItem
      Caption = 'Gravar'
      ShortCut = 113
      OnClick = bgravarClick
    end
    object Fechar1: TMenuItem
      Caption = 'Fechar'
      ShortCut = 16499
      OnClick = Fechar1Click
    end
  end
end
