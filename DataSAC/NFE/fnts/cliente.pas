...
uses
  ...
  Mask, rxtooledit, rxcurredit, maskutils, RXDBCtrl, PageView, wwdbedit,
...
uses modulo, principal, loc_regiao, regiao, loc_funci, 
  ...
  cliente_inicio, xloc_cidade, xloc_cnae, webcam2, principal2;
...
begin
  ...
  IF FRMMODULO.QRCLIENTE.FIELDBYNAME('situacao').AsInteger = 2 THEN wwDBComboBox1.Text := 'CADASTRO EM OBSERVAÇÃO';
...
begin
  ...
  BEGIN
    ...
    frmmodulo.qrcliente.fieldbyname('codigo').asstring := frmprincipal2.codifica('000007');
...
begin
  if frmprincipal2.autentica('Excluir cliente',4) then
  begin
    ...
    begin
      ...
      begin
       ...
       begin
            frmprincipal2.log(codigo_usuario,'CLIENTE',frmmodulo.qrcliente.fieldbyname('codigo').asstring,'EXCLUIU',frmmodulo.qrcliente.fieldbyname('NOME').asstring);
...
begin
  ...
  IF CONTINUA_INCLUSAO THEN
    frmprincipal2.agenda('1','CLIENTE',DBEdit2.Text,DBEdit10.Text,DBEdit11.Text,'',DBEdit12.Text,DBEdit13.Text,'','','1');
    frmprincipal2.log(codigo_usuario,'CLIENTE',frmmodulo.qrcliente.fieldbyname('codigo').asstring,'INCLUIU',frmmodulo.qrcliente.fieldbyname('NOME').asstring);
  //else if (frmmodulo.qrcliente.State = dsedit) then
  //  frmprincipal2.agenda('2','CLIENTE',DBEdit2.Text,DBEdit10.Text,DBEdit11.Text,'',DBEdit12.Text,DBEdit13.Text,'','','1');
  ...
  if (frmmodulo.qrcliente.State = dsinsert) or (frmmodulo.qrcliente.State = dsedit) then
    begin
        ...
        begin
          ...
          frmprincipal2.qrmestre.sql.add('update c000000 set sequencia = sequencia - 1 where codigo = ''000007''');
...
begin
  ...
  IF (frmmodulo.qrcliente.state = dsinsert) or (frmmodulo.qrcliente.State = dsedit) then
  begin
    frmmodulo.qrcliente.fieldbyname('codregiao').asstring := frmprincipal2.zerarcodigo(dbedit28.text,6);
    if dbedit28.text <> '000000' then
      if not FrmPrincipal2.Locregistro(frmmodulo.qrregiao,dbedit28.text,'codigo') then dbedit28ButtonClick(Frmcliente) else dbedit29.setfocus
    else
...
begin
  ...
  IF (frmmodulo.qrcliente.state = dsinsert) or (frmmodulo.qrcliente.State = dsedit) then
  begin
    frmmodulo.qrcliente.fieldbyname('codvendedor').asstring := frmprincipal2.zerarcodigo(dbedit29.text,6);
    if dbedit29.text <> '000000' then
    BEGIN
      if not FrmPrincipal2.Locregistro(frmmodulo.qrfunci,dbedit29.text,'codigo') then
...
begin
  ...
  BEGIN
    ...
    begin
      ...
      begin
        //if Length(frmprincipal2.somenteNumero(dbedit15.Text)) > 11 then
        begin
     ...
     begin
       ...
       BEGIN
             ...
             qrpgto.close;
              ...
              begin
                memo1.lines.add(qrpgto.fieldbyname('data').asstring+' -->'+frmPrincipal2.texto_justifica(formatfloat('###,###,##0.00',qrpgto.fieldbyname('valor_pago').asfloat),12,' ','E'));
                qrpgto.Next;
              end;
                  PPGTO.Visible := TRUE;
       END;
...
begin
  ...
  IF (frmmodulo.qrcliente.state = dsinsert) or (frmmodulo.qrcliente.State = dsedit) then
  begin
    frmmodulo.qrcliente.fieldbyname('codconvenio').asstring := frmprincipal2.zerarcodigo(ecodconvenio.text,6);
    if ecodconvenio.text <> '000000' then
    begin
      if not FrmPrincipal2.Locregistro(frmmodulo.qrconvenio,ecodconvenio.text,'codigo') then ecodconvenioButtonClick(Frmcliente)
      else
      begin
...
begin
  ...
  begin
    ...
    CAMPO := PANSICHAR(FRMPRINCIPAL2.somenteNumero(FRMMODULO.QRCLIENTE.FIELDBYNAME('CPF').ASSTRING));
  end
  else
  begin
    ...
    begin
      tipo := 'IE';
      CAMPO := PANSICHAR(FRMPRINCIPAL2.somenteNumero(FRMMODULO.QRCLIENTE.FIELDBYNAME('RG').ASSTRING));
    end
    else
    begin
...
if frmPrincipal2.ChecaEstado(euf.Text) = false then
  begin
...
begin
  if frmprincipal2.autentica('Alterar Cadastro',4) then
  begin
    pcad.enabled := true;
    bcad.Visible := false;
    pcad.Caption := 'Informações Cadastrais';
...
begin
tedit(sender).Color := clwindow;
frmmodulo.qrcliente.fieldbyname('numero').asstring := frmprincipal2.zerarcodigo(dbedit38.text,5);
end;
