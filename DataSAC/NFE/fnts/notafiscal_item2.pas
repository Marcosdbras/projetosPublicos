...
uses
  ...
  RzBtnEdt, Menus, DB, MemDS, DBAccess, IBC, RzButton, RzRadChk, RzPanel,
...
uses modulo, principal, venda_serial, NotaFiscal_Item_Serial, xloc_produto,
...
begin
  ...
  begin
    ...
    FrmNotaFiscal.qrnotafiscal_item.Params.Parambyname('CODLANCAMENTO'       ).asstring := frmprincipal2.codifica('000032');
    ...
    frmNotaFiscal.qrnotafiscal_item.Params.Parambyname('codigo'              ).asstring := frmprincipal2.codifica('000062');
  ...
  begin
    ...
    if frmnotafiscal.check_desconto.Checked = false then
      begin
       ...
       //frmnotafiscal_menu.qrnota.FieldByName('VALOR_PRODUTOS').asfloat := frmprincipal2.Arredondar(vtot_produtos,2);
       frmnotafiscal_menu.qrnota.FieldByName('VALOR_PRODUTOS').asfloat := vtot_produtos;

       //ShowMessage(FloatToStr(vtot_produtos)+ '  ' +FloatToStr(frmnotafiscal_menu.qrnota.FieldByName('VALOR_PRODUTOS').asfloat));

       //frmprincipal2.Arredondar(
       //(frmnotafiscal_menu.qrnota.FieldByName('VALOR_PRODUTOS').asfloat +
       //ed_subtotal.value)-valor_anterior,2);
      end
    else
      begin
    ...
    if ed_desconto.Value <> vdesconto_anterior then
      ...
      frmnotafiscal_menu.qrnota.FieldByName('TOTAL_NOTA').asfloat := //frmprincipal2.Arredondar(
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {ed_pis_valor.value:= 0;
        ed_cofins_valor.value:= 0;
        }
      end
      else
      begin
...
procedure TfrmNotaFiscal_Item2.ed_csosnKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key = #13 then
  begin
