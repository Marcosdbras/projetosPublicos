...
uses
  ...
  CurrEdit, RXCtrls, Menus, RpDefine, RpCon, RpConDS, RzEdit, RzDBEdit,
...
uses modulo, loc_cliente, principal, loc_funci, Math, pedido_impressao,
  ...
  ped_fechamento, pedido_serial, pedido_grade;
...
begin
  ...
  frmmodulo.qrPedido.SQL.Add('select * from c000056P order by codigo');
  ...
  begin
    ...
    qrpedido_produto.sql.Add('select * from c000057P where codpedido = '''+frmmodulo.qrPedido.fieldbyname('codigo').asstring+'''');
...
begin
  ...
  frmmodulo.qrPedido.fieldbyname('codigo').asstring := frmprincipal.codifica('000056P');
  pficha.Enabled := true;
...
begin
  ...
  begin
    ...
    qrpedido_produto.close;
    qrpedido_produto.sql.clear;
    qrpedido_produto.sql.Add('delete from c000057P where codpedido = '''+frmmodulo.qrPedido.fieldbyname('codigo').asstring+'''');
  ...
  begin
    ...
    begin
      ...
      begin
       ...
       qrquery2.close;
       qrquery2.sql.clear;
       qrquery2.sql.add('delete from c000060P where codpedido = '''+frmmodulo.qrPedido.fieldbyname('codigo').asstring+'''');
       qrquery2.execsql;
