...
uses modulo, principal, os, os_serial;
...
procedure Tfrmos_item.rdesconto2KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure Tfrmos_item.ecodigoKeyPress(Sender: TObject; var Key: Char);
      ...
      begin
        ...
        RQTDE.Enabled := TRUE;
          ...
          begin
            ...
            frmos.qrproduto_serial.sql.Add('select * from c000022 where codproduto = '''+frmmodulo.qrproduto.fieldbyname('codigo').asstring+''' and situacao = 1 order by codigo');
...
procedure Tfrmos_item.eprodutoChange(Sender: TObject);
begin
...
procedure Tfrmos_item.EPRODUTOKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = vk_down then
  begin
...
procedure Tfrmos_item.wwDBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
...
procedure Tfrmos_item.wwDBGrid1Enter(Sender: TObject);
begin
//  VER_PRODUTO := TRUE;
//  PPRODUTO.VISIBLE := TRUE;
end;

procedure Tfrmos_item.ecodigoEnter(Sender: TObject);
begin
  TEDIT(SENDER).COLOR := CLYELLOW;
  PPRODUTO.VISIBLE := FALSE;
end;

procedure Tfrmos_item.ecodigoExit(Sender: TObject);
begin
  TEDIT(SENDER).COLOR := CLWINDOW;
end;
...
begin
  ...
  begin
    ...
    frmos.qros_produto.FieldByName('codproduto').asstring := frmmodulo.qrproduto.fieldbyname('codigo').asstring;
    frmos.qros_produto.FieldByName('unidade').asstring := frmmodulo.qrproduto.fieldbyname('UNIDADE').asstring;
...
procedure Tfrmos_item.wwDBGrid1DblClick(Sender: TObject);
begin
    eproduto.text := frmmodulo.qrproduto.fieldbyname('produto').asstring;
    ecodigo.text  := frmmodulo.qrproduto.fieldbyname('codigo').asstring;
