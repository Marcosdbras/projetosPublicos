...
uses modulo, principal, venda, venda_grade, venda_serial,
  loc_produto_venda_auto, xloc_produto, ecf, os,
  os_serial;
      ...
      begin
       ...
       begin
          ...
          begin
            ...
            BEGIN
              frmos.qrproduto_grade.close;
              frmos.qrproduto_grade.sql.clear;
              frmos.qrproduto_grade.sql.Add('select * from c000021 where codigo = '''+cod_serial+'''');
              frmos.qrproduto_grade.open;
              if frmos.qrproduto_grade.RecordCount > 0 then
              begin
                lserial.Caption := frmos.qrproduto_grade.fieldbyname('numeracao').asstring;
                serial_grade := 2;
                codigo_serial_grade := frmos.qrproduto_grade.fieldbyname('codigo').asstring;
                RQTDE.SETFOCUS;
              end
              else
              begin
            ...
            BEGIN
              ...
              frmos.qrproduto_grade.sql.clear;
              if estoque_negativo then
                frmos.qrproduto_grade.sql.Add('select * from c000021 where codproduto = '''+qrproduto.fieldbyname('codigo').asstring+''' order by codigo')
              else
                frmos.qrproduto_grade.sql.Add('select * from c000021 where codproduto = '''+qrproduto.fieldbyname('codigo').asstring+''' and estoque > 0 order by codigo');
              frmos.qrproduto_grade.open;
