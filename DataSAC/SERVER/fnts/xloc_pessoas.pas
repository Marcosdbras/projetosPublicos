...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000070 where codigo is not null '+parametro_pesquisa+' ORDER BY nome');
  query.open;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000070 where upper('+tipo+') like '''+ref+loc.Text+'%'''+parametro_pesquisa+' order by nome');
    query.open;
  except
  end;
end;
