...
procedure Tfrmxloc_CONTA.FormClose(Sender: TObject;
...
procedure Tfrmxloc_CONTA.locKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = vk_down then
  begin
...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000035 where codigo is not null '+parametro_pesquisa+' ORDER BY CLASSIFICACAO');
  query.open;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000035 where upper('+tipo+') like '''+ref+loc.Text+'%'''+parametro_pesquisa+' order by classificacao');
    query.open;
  except
  end;
end;
