...
uses
  ...
  RzDBEdit, RzDBBnEd, AdvShapeButton, IBCustomDataSet;
...
uses modulo, locvendedor, principal, loc_setor,
  ...
  xloc_cliente, os_item_2,  os_item_2_auto, loc_fornecedor, os_fechamento_auto;
...
begin
  ...
  if combosituacao.Text = 'FECHAR - Executa o Fechamento' then
...
procedure TfrmOS_Auto.blocveiculoClick(Sender: TObject);
begin
...
procedure TfrmOS_Auto.blocterceiroClick(Sender: TObject);
begin
...
procedure TfrmOS_Auto.DBEdit21Change(Sender: TObject);
begin
  if (frmmodulo.qros.State = dsinsert) or (frmmodulo.qros.State = dsedit) then
  begin
   frmmodulo.qros.fieldbyname('TERCEIRO_TOTAL').asfloat:=frmmodulo.qros.fieldbyname('TERCEIRO_VALOR').asfloat - frmmodulo.qros.fieldbyname('TERCEIRO_COMISSAO').asfloat;
  end;
end;

procedure TfrmOS_Auto.DBEdit22Change(Sender: TObject);
begin
  if (frmmodulo.qros.State = dsinsert) or (frmmodulo.qros.State = dsedit) then
  begin
   frmmodulo.qros.fieldbyname('TERCEIRO_TOTAL').asfloat:=frmmodulo.qros.fieldbyname('TERCEIRO_VALOR').asfloat - frmmodulo.qros.fieldbyname('TERCEIRO_COMISSAO').asfloat;
