...
begin
  ...
  BEGIN
  query.SQL.Add('select * from c000004 where codigo Is NOT NULL ORDER BY filial');
  query.open;
  LOC.SETFOCUS;
  END
  ELSE
  BEGIN
  query.SQL.Add('select * from c000004 where codigo Is NOT NULL AND UPPER(filial) LIKE '''+ANSIUPPERCASE(PARAMETRO_PESQUISA)+'%'' ORDER BY filial');
  query.open;
  GRID.SetFocus;
  END;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000004 where upper('+tipo+') like '''+ref+loc.Text+'%'' order by '+tipo);
    query.open;
  except
  end;
end;
