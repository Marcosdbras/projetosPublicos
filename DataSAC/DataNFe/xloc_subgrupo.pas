...
procedure Tfrmxloc_subgrupo.FormClose(Sender: TObject;
...
procedure Tfrmxloc_subgrupo.locKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if key = vk_down then
  begin
...
begin
  ...
  query.Close;
  query.SQL.clear;
  query.SQL.Add('select * from c000018 where codigo IS NOT NULL '+PARAMETRO_PESQUISA+' ORDER BY SUBGRUPO');
  query.open;
end;
...
begin
  ...
  try
    ...
    if loc.text = '' then
      ...
      query.sql.Add('select * from c000018 where upper('+tipo+') like '''+ref+loc.Text+'%'''+PARAMETRO_PESQUISA+' order by '+tipo);
    query.open;
  except
  end;
end;
